...
static void smc911x_handle_mac_address(struct eth_device *dev)
{
	...
	/*
	 * If "ethaddr" is set in the environment variables,
	 * give preference to it over the MAC address obtained
	 * from optional eeprom.
	 */
	if (eth_getenv_enetaddr("ethaddr", envaddr))
		m = envaddr;
...
#ifdef CONFIG_SMC911X_PHY_CONFIGURE
...
#endif /* CONFIG_SMC911X_PHY_CONFIGURE */
...
static void smc911x_halt(struct eth_device *dev)
{
	...
	/*
	 * Be sure to let an intended MAC address loaded into ADDR[HL]
	 * registers beyond the software reset, even if it's not used by
	 * U-Boot itself.
	 *
	 * This treatment is requried not for U-Boot, but for the Linux
	 * kernel to probe and make use of the same MAC address as the
	 * one used by U-Boot.
	 */
	smc911x_handle_mac_address(dev);
}
