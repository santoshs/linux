...
#define MMC_RD_BLK(mmc) (mmc->read_bl_len)
#define RD_START_BLK(mmc, bytes) ( (bytes) / MMC_RD_BLK(mmc) )
#define RD_NUM_BLKS(mmc, bytes) ( (bytes + MMC_RD_BLK(mmc) - 1) \
		/ MMC_RD_BLK(mmc) )

extern void do_booti_linux (boot_img_hdr *hdr);
extern int mmc_read_blocks(struct mmc *mmc, void *dst, ulong start, lbaint_t blkcnt);

/* booti [<addr> | [ <mmc> [ <partition> ] ] ] */
int do_booti (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned addr;
	char *ptn = "boot";
	struct mmc *mmcc = NULL;
	...
	if (mmcc) {
		...
		if (mmc_init(mmcc)) {
			printf("mmc%p init failed\n", mmcc);
			goto fail;
		}
		if (mmc_read_blocks(mmcc, hdr, pte->start, 1) < 0) {
		...
		sector = pte->start + RD_NUM_BLKS(mmcc, hdr->page_size);
		if (mmc_read_blocks(mmcc, (void*) hdr->kernel_addr, sector,
			     RD_NUM_BLKS(mmcc, hdr->kernel_size)) < 0) {
			printf("booti: failed to read kernel\n");
			goto fail;
		}

		sector += RD_NUM_BLKS(mmcc,
				ALIGN(hdr->kernel_size, hdr->page_size));
		if (mmc_read_blocks(mmcc, (void*) hdr->ramdisk_addr, sector,
			     RD_NUM_BLKS(mmcc, hdr->ramdisk_size)) < 0) {
