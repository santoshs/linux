...
/**
 * Emmc_Write_Multi - eMMC Write Multi Sector
 ...
 */
RC Emmc_Write_Multi(uchar* pBuff, ulong start_sector, ulong sector_count)
{
	RC	ret = EMMC_SUCCESS;
	...
	{
		emmc_Make_Cmd(25, start_sector, (ulong *)pBuff, sector_count << EMMC_SECTOR_SIZE_SHIFT);
	}
	else
	{
		emmc_Make_Cmd(25, start_sector << EMMC_SECTOR_SIZE_SHIFT, (ulong *)pBuff, sector_count << EMMC_SECTOR_SIZE_SHIFT);
	}
...
/**
 * Emmc_Write_Single - eMMC Write Single Sector
 * @return EMMC_SUCCESS   : Success
 *         EMMC_ERR_CARD_BUSY     : Card status busy
 *         EMMC_ERR_CMD_ISSUE     : CMD Issue Error
 *         EMMC_ERR_BUFFER_ACCESS : Buffer Access Error
 ...
 *         EMMC_ERR_STATE         : State Error
 */
RC Emmc_Write_Single(uchar* pBuff, ulong start_sector)
{
	RC	ret = EMMC_SUCCESS;
	...
	if (gDrv_Info.access_mode == EMMC_ACCESS_MODE_SECTOR)
	{
		emmc_Make_Cmd(24, start_sector, (ulong *)pBuff, EMMC_BLOCK_LENGTH);
	}
	else
	{
		emmc_Make_Cmd(24, start_sector<<EMMC_SECTOR_SIZE_SHIFT, (ulong *)pBuff, EMMC_BLOCK_LENGTH);
	}
	ret = emmc_Issue_Cmd();
	if (ret != EMMC_SUCCESS)
	{
