...
/**
   ...
   @param[in]  privKey_ptr    - temp buffer in RAM for random generating or providing the
                                private key.
...

{
   ...
   DxUint32_t            *order_ptr;     /* pointer to order of EC generator */
   ...
   orderSizeInBits = LLF_EC_DOMAINS_ARRAY[DomainID].EC_OrderSizeInBits;
    ...
    /* pointer to order of EC generator  as integer buffer */
   order_ptr = (DxUint32_t *)LLF_EC_DOMAINS_ARRAY[DomainID].EC_GenerOrder;
   ...
   if( isPrivKeyInternal == DX_TRUE )
   {
	   ...
	   Error = CRYS_RND_GenerateVectorInRange( orderSizeInBits, (DxUint8_t*)order_ptr, privKey_ptr );
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( rS/*dstReg*/, LenID_r+1, privKey_ptr/*src_ptr*/,
                                 (orderSizeInBits+7)/8, VirtualHwBaseAddr );

   /* set EC generator into point W for scalar multiplication */
   LLF_ECPKI_SetPoint(
                     (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Gener_X,
                     (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Gener_Y,
                     DX_NULL/* Z_ptr */,
                     4*LLF_EC_DOMAINS_ARRAY[DomainID].EC_ModulusSizeInWords,
                     LenID_n,
					...
					DxUint32_t                 VirtualHwBaseAddr /*in*/ )
{
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( rOrd/*dstReg*/, LenID_r+1,
                                   ...
                                   4*LLF_EC_DOMAINS_ARRAY[DomainID].EC_OrderSizeInWords , VirtualHwBaseAddr );
