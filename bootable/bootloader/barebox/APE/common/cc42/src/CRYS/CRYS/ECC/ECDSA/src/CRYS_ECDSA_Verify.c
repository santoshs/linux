								   ...
								   CRYS_ECPKI_HASH_OpMode_t         HashMode               /*in*/ )
{
   ...
   PublKey_ptr = (CRYS_ECPKI_PublKey_t *)(SignerPublKey_ptr->PublKeyDbBuff);     
   ...
   ((CRYS_ECPKI_PublKey_t*)(ccmWorkingContext_ptr->ECDSA_SignerPublKey.PublKeyDbBuff))->DomainID = 
	    ((CRYS_ECPKI_PublKey_t*)(SignerPublKey_ptr->PublKeyDbBuff))->DomainID;
							 ...
							 DxUint32_t                      SignatureSizeBytes      /*in*/  )
{
   ...
   PublKey_ptr = (CRYS_ECPKI_PublKey_t *)ccmWorkingContext_ptr->ECDSA_SignerPublKey.PublKeyDbBuff; 
   ...
   /* ----------------------------------------------------------------------------- */
    ...
    /* Temp buffers */
    SignatureC_ptr       = ccmWorkingContext_ptr->LLF.Tempbuff;
    SignatureD_ptr       = SignatureC_ptr + ModSizeInWords + 1; /* Max lengths of C in whole words */
    MessageRepresent_ptr = SignatureD_ptr + ModSizeInWords + 1 ;
	TempBuff_ptr         = MessageRepresent_ptr + ModSizeInWords + 1 ;

 	/* Clean memory  */
    DX_VOS_MemSetZero((DxUint8_t*)SignatureC_ptr, 2*4*(ModSizeInWords + 1));
	...
	/* If DomainID = WMDRM then copy reversed D,C of length OrderSizeInBytes each one */
    if(DomainID == CRYS_ECPKI_DomainID_WMDRM10)
	{
		/* Reverse signature to little endian   */
		CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)SignatureD_ptr,
									SignatureIn_ptr, OrderSizeInBytes );
		CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)SignatureC_ptr, 
									SignatureIn_ptr + OrderSizeInBytes, OrderSizeInBytes );		
    } 
	else /* For other domains decode and copy signature according to ECDSA standard */
	{
        /* Temporary we simly copying reversed C,D strings of length OrderSizeInBytes */
		CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)SignatureC_ptr,
									SignatureIn_ptr, OrderSizeInBytes );
		CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)SignatureD_ptr, 
									SignatureIn_ptr + OrderSizeInBytes, OrderSizeInBytes );
