...
/* ------------------------------------------------------------
**
* @brief This function executes a revers byte copy, including on the same place.
*
* @param[in] dst_ptr - The first counter (destination)buffer (may be the same as src_ptr).
* @param[in] src_ptr - The second counter (source) buffer. 
* @param[in] size    - The second counter size in bytes.
*
*/
static void CRYS_ECPKI_ReverseMemcpy( DxUint8_t *dst_ptr , DxUint8_t *src_ptr , DxUint32_t size )
{
					...
					CRYS_EC_ELGAMAL_TempData_t     *TempData_ptr             /*in*/ )
{
   ...
   DxUint32_t ModSizeInBytes, OrderSizeInBytes;
   ...
   RETURN_IF_ECPKI_UNSUPPORTED( ReceiverUserPublKey_ptr, MessageIn_ptr,                                 
                                ...
                                ModSizeInBytes, OrderSizeInBytes, Error, 
   ...
   ReceiverPublKey_ptr = (CRYS_ECPKI_PublKey_t*)(ReceiverUserPublKey_ptr->PublKeyDbBuff);
   /* Set DomainID */
   ...
   /* if message size is greater than (EC modulus size â€“ 1) return error */
   ...
   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)TempData_ptr->LLF.m,  MessageIn_ptr, MessageInSizeBytes );
   TempData_ptr->LLF.mSizeBytes = MessageInSizeBytes;

   if(IsEphemerKeyInternal != 1)
   {   	     
		/*  Convert ephemeral private key to little endianness and set it into TempData */
		CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)TempData_ptr->LLF.EphemerPrivKey, 
		                            EphemerPrivKeyIn_ptr, EphemerPrivKeySizeBytes );
		TempData_ptr->LLF.EphemerPrivKeySizeBytes = EphemerPrivKeySizeBytes;
   }
   ...
   Error = LLF_ECPKI_ELGAMAL_Encrypt( 
	                            ReceiverPublKey_ptr, EncrMessageOut_ptr, 
   ...
   if(Error == CRYS_OK)
   {
	    ...
	    CRYS_ECPKI_ReverseMemcpy( EncrMessageOut_ptr, EncrMessageOut_ptr/*Vx*/, ModSizeInBytes );
		CRYS_ECPKI_ReverseMemcpy( EncrMessageOut_ptr + ModSizeInBytes/*Vy*/, 
			                       EncrMessageOut_ptr + ModSizeInBytes, ModSizeInBytes );
		CRYS_ECPKI_ReverseMemcpy( EncrMessageOut_ptr + 2*ModSizeInBytes/*Bx*/, 
			                       EncrMessageOut_ptr + 2*ModSizeInBytes, ModSizeInBytes );
		CRYS_ECPKI_ReverseMemcpy( EncrMessageOut_ptr + 3*ModSizeInBytes/*By*/, 
			                       EncrMessageOut_ptr + 3*ModSizeInBytes, ModSizeInBytes );
   }
...
{				
   ...
   RETURN_IF_ECPKI_UNSUPPORTED( ReceiverUserPrivKey_ptr, EncrMessageIn_ptr,                                 
								...
								TempData_ptr, ReceiverPrivKey_ptr, Error, Error,
								DomainID, DomainInfo.EC_ModulusSizeInBits, ModSizeInBytes, Error, Error, Error, 								
                                Error, Error, Error, Error, Error, Error,  Error );
   ...
   ReceiverPrivKey_ptr = (CRYS_ECPKI_PrivKey_t*)(ReceiverUserPrivKey_ptr->PrivKeyDbBuff);
   ...
   Error = LLF_ECPKI_ELGAMAL_Decrypt ( ReceiverPrivKey_ptr, EncrMessageIn_ptr, 
			   	                       DecrMessageOut_ptr, *DecrMessageOutSize_ptr, TempData_ptr );
   if(Error != CRYS_OK)
   {
	   DX_VOS_MemSetZero( DecrMessageOut_ptr, *DecrMessageOutSize_ptr );
   	   goto END;
   }

   /* reverse the decrypted message */
   CRYS_ECPKI_ReverseMemcpy( DecrMessageOut_ptr, DecrMessageOut_ptr, *DecrMessageOutSize_ptr );
