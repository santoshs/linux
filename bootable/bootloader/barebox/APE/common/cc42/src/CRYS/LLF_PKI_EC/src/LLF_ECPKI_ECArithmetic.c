				       ...
				       DxUint32_t                 VirtualHwBaseAddr    /*in*/ )
{
   ...
   if( countTemps < 7 )
	...
	else/* Else output PointRes = (1,1,0) and stop, that means: result on infinity */
	{
				       ...
				       DxUint32_t                 VirtualHwBaseAddr    /*in*/ )
{
   ...
   DxUint8_t  *A_ptr; /* pointer to EC coefficient A */
   ...
   if( countTemps < 6 )
	...
	A_ptr = (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_A;

	/* copy EC parameter A into register rT4 */
    LLF_PKI_PKA_CopyDataIntoPkaReg( rT4/*dstReg*/, LenID_n+1, A_ptr/*src_ptr*/,
				       ...
				       DxUint32_t                     VirtualHwBaseAddr /*in*/ )
{
...

	{
	    ...
	    LLF_PKI_PKA_CopyDataIntoPkaReg( rT/*dstReg*/, LenID_r+1,
	                      (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_GenerOrder/*src_ptr*/,
	                      4*rSizeInWords, VirtualHwBaseAddr );
...

{
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg( rA/*dstReg*/, LenID_n+1,
                      (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_A/*src_ptr*/,
                      modSizeInBytes , VirtualHwBaseAddr );

    LLF_PKI_PKA_CopyDataIntoPkaReg( rT2/*dstReg*/, LenID_n+1,
                      (DxUint8_t*)LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_B/*src_ptr*/,
                      modSizeInBytes , VirtualHwBaseAddr );
...

{
	...
	if( ((DxUint8_t*)P_ptr)[modSizeInBytes-1] > 100 )
	{
	    changedByteNum = (modSizeInBytes-1) % 4;
		addValue = 0x00000001 << 8*changedByteNum;
		/* set 0 into changed byte */
        *((DxUint8_t*)&changedWord + changedByteNum) = 0;
	...
	else
	{
		...
		LLF_PKI_PKA_SetWordIntoRegister( LenID_n, 0/*changedWord*/, PAff_ptr->rX, changedWordNum/*i*/, VirtualHwBaseAddr );
            ...
            changedWordNum--;
            changedWord = LLF_PKI_PKA_GetWordFromRegister( LenID_n,  PAff_ptr->rX, changedWordNum, VirtualHwBaseAddr );
			addValue = 0x00000001 << 24;
			/* set 0 into changed byte */
	        *((DxUint8_t*)&changedWord + 3) = 0;
        }
		...
		changedByteNum = (modSizeInBytes-1) % 4 - 1;
	        ...
	        *((DxUint8_t*)&changedWord + changedByteNum) = 0;
	        *((DxUint8_t*)&changedWord + changedByteNum + 1) = 0;
        }
	}
...

{
    ...
    LLF_PKI_PKA_CopyDataIntoPkaReg( rT1/*dstReg*/, LenID_n+1,
                      (DxUint8_t*)(LLF_EC_DOMAINS_ARRAY[DomainID].EC_Param_B)/*src_ptr*/,
                      4*LLF_EC_DOMAINS_ARRAY[DomainID].EC_ModulusSizeInWords, VirtualHwBaseAddr );

	/* If Z = 0 (point on infinity), then output:
	       if (B != 0)  output (0,0)
	       else         output (0,1),     where B - parameter of elliptic curve.  */
...
/**
   ...
   @param[in]  sizeInBytes - Size of coordinates in bytes.
