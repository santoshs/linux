                             ...
                             DxUint32_t                VirtualHwBaseAddr /*in*/)
{
   ...
   //PRINTF("Entered LLF_PKI_PKA_SetRegsSizesTab\n");
   /* FUNCTION LOGIC */
   ...
   if( IsDefaultMap == 0 )
   {
	   ...
	   for( i = 0; i < CountOfSizes; i++ )
	   {
                 //PRINTF("set sizes into PKA registers sizes table started\n");
		  CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_SIZES_TAB_ADDR, RegsSizes_ptr[i] );
                 //PRINTF("set sizes into PKA registers sizes table completed\n");
	   }
   }
   ...
   else
   {
	   ...
	   MaxSizeWords = ( MaxSizeBits + 31 ) / 32;
          //PRINTF("Write1 starts\n");
	   /* write exact size into first table entry */
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_SIZES_TAB_ADDR, MaxSizeBits );
            //PRINTF("Write1 completed\n");


	   /* write size with extra word into tab[1] = tab[0] + 32 */
            //PRINTF("Write2 starts\n");
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_SIZES_TAB_ADDR + 4,
	                                32*MaxSizeWords + 32 );
               // PRINTF("Write2 completed\n");
   ...
   {
           //PRINTF("Write3 starts\n");
	  CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_SIZES_TAB_ADDR + 4*i, PKA_SIZE_ENTRY_NOT_USED );
          //PRINTF("Write3 completed\n");
   }
   //PRINTF("Completed LLF_PKI_PKA_SetRegsSizesTab\n");
   return Error;
                          ...
                          DxUint32_t              VirtualHwBaseAddr   /*in*/ )
{
   ...
   if( IsDefaultMap == 1 )
   {
	   ...
	   for( i = 0; i < 32-2; i++ )
	   {
		  ...
		  {
			  CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_MAP_TAB_ADDR + 4*i,
			                               CurrentAddr );
	   ...
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_MAP_TAB_ADDR + 4*30,
		                            CurrentAddr );

	   CurrentAddr = CurrentAddr + defaultMaxSize/8;
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_MAP_TAB_ADDR + 4*31,
		                            CurrentAddr );
   ...
   if( IsDefaultMap == 0 )
   {
	   ...
	   for( i = 0; i < *CountOfRegs; i++ )
	   {
		  CRYS_PLAT_SYS_WriteRegister(
		          VirtualHwBaseAddr + LLF_PKI_PKA_REGS_MAP_TAB_ADDR + 4*RegsMap_ptr->RegsNum[i],
		          RegsMap_ptr->RegsAddr[i] );
	   }

	   /* set virtual addresses of N,NP,T0,T1 registers into N_NP_T0_T1_Reg */
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_N_NP_T0_T1_REG_ADDR, N_NP_T0_T1 );
   }
 ...
 */
                 ...
                 DxUint32_t               VirtualHwBaseAddr   /*in*/ )
 {
  ...
  //PRINTF("Entered LLF_PKI_PKA_InitPKA \n");
   /* FUNCTION LOGIC */
   ...
   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_HW_PKA_CLK_ENABLE_ADDR , 0x1UL );
   //PRINTF("Enabling PKA clock completed\n");
  ...
  /* LLF_PKI_PKA_Set_StartMemAddr_Reg (LLF_PKI_PKA_START_MEM_ADDR , VirtualHwBaseAddr); */
   ...
   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_REGS_SIZES_TAB_ADDR + 3*4,
                                32*RegSizeWords );

   //PRINTF("Set size of register into RegsSizesTable successful\n");
   /* clean PKA data memory */
   LLF_PKI_PKA_ClearBlockOfRegs( 0/* FirstReg*/, CountOfRegs-2,
                                     3/*LenID*/, VirtualHwBaseAddr );

  //PRINTF("Clean of PKA data memory successful\n");
   /* clean temp PKA registers 30,31 */
   LLF_PKI_WAIT_ON_PKA_DONE( VirtualHwBaseAddr );
   //PRINTF("Wait on PKA Done completed\n");
   LLF_PKI_PKA_GetRegAddress(30/*VirtReg*/,addr/*physAddr*/, VirtualHwBaseAddr);
   //PRINTF("Physical address obtained\n");
   LLF_PKI_HW_CLEAR_PKA_MEM( VirtualHwBaseAddr , addr , RegSizeWords );
  //PRINTF("Clear PKA memory completed\n");
   LLF_PKI_PKA_GetRegAddress(31/*VirtReg*/,addr/*physAddr*/, VirtualHwBaseAddr);
   //PRINTF("Get phy address\n");
   LLF_PKI_HW_CLEAR_PKA_MEM( VirtualHwBaseAddr , addr , RegSizeWords );
   //PRINTF("Clear PKA memory completed\n");

   /* ......  End of function ...... */
   //PRINTF("Ended LLF_PKI_PKA_InitPKA \n");
   return Error;
 ...
 */
 void LLF_PKI_PKA_FinishPKA( DxUint32_t VirtualHwBaseAddr /*in*/ )
 {
   ...
   /* close PKA clocks */
   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_HW_PKA_CLK_ENABLE_ADDR , 0x0UL );
                                         ...
                                         DxUint32_t  VirtualHwBaseAddr )
{
   ...
   /*--------------------------------------------------------------------*/
   Error = LLF_PKI_PKA_DivLongNum(  LenID,        /*LenID*/
                                    rT0,          /*opA*/
			                        S,            /*shift*/
			                        0,            /*opB = N*/
			                        1,            /*Res NP*/
		                        ...
		                        DxUint32_t     VirtualHwBaseAddr )
{
   ...
   for( i = 0; i < nWords; i++ )
   {
        ...
        LLF_PKI_PKA_CopyDataFromPkaReg(
                                (DxUint8_t*)tempRes/*dst_ptr*/, RegSizeBytes,
                                rT1/*srcReg*/,
                                VirtualHwBaseAddr );
      #endif
                          ...
                          DxUint32_t         VirtualHwBaseAddr  /*in*/ )
{
  ...
  //PRINTF("Entered LLF_PKI_CalcNpAndInitModOp\n");
  #ifdef LLF_PKI_PKA_DEBUG
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( 0/*dstReg*/, 1/*LenID*/,
   ...
   if( NpCreateFlag == PKA_SetNP )
   {
      ...
      LLF_PKI_PKA_CopyDataIntoPkaReg( 1/*dstReg*/, 1/*LenID*/, (DxUint8_t*)NP_ptr/*src_ptr*/,
   ...
   else
   {
	   ...
	   /* output of NP value */
	   LLF_PKI_PKA_CopyDataFromPkaReg(
	                  (DxUint8_t*)NP_ptr/*dst_ptr*/, 4*LLF_PKI_PKA_BARRETT_MOD_TAG_BUFF_SIZE_IN_WORDS,
                       1/*srcReg*/, VirtualHwBaseAddr );
   }

   /* End of the function */
   //PRINTF("Ended LLF_PKI_CalcNpAndInitModOp \n");
   return Error;
                                ...
                                DxUint32_t      VirtualHwBaseAddr  /*in*/ )
 {
   ...
   if( Opcode == PKA_ModInv || Opcode == PKA_Div)
   {
	  ...
	  OpT = OpB;
	   ...
	   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_OPCODE_REG_ADDR , FullOpCode );
   ...
   switch( Opcode )
   {
     ...
     default:
           ...
           ResDiscard == 0 && Opcode != PKA_Terminate )
       {
	       ...
	       LLF_PKI_PKA_CopyDataFromPkaReg(
	                                (DxUint8_t*)tempRes/*dst_ptr*/, 4*OpSizeWords,
	       ...
	       if( Opcode == PKA_Div || Opcode == PKA_ModInv )
	       {
			if( Opcode == PKA_Div )
		        ...
		        LLF_PKI_PKA_CopyDataFromPkaReg(
	                                (DxUint8_t*)tempRes1/*dst_ptr*/, 4*OpSizeWords,
...
//#if (!defined(_INTERNAL_CRYS_NO_RSA_KG_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_CRT_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_DECCRYPT_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_SIGN_SUPPORT)) || defined(_INTERNAL_CRYS_BASE_PKI_IN_ROM)
/***********      LLF_PKI_PKA_CopyDataIntoPkaReg function   *******************/
/**
 * @brief This function  revertive copies source data into PKA register.
 *
 ...
 *
 */
 //void LLF_PKI_PKA_ReverseCopyDataIntoPkaReg(
   //                             DxInt8_t    dstReg,            /*out*/
     //                           DxUint8_t   LenID,             /*in*/
       //                         DxUint8_t  *src_ptr,           /*in*/
...
//	   LLF_PKI_HW_REVERSE_LOAD_BLOCK_TO_PKA_MEM( VirtualHwBaseAddr, currAddr, (DxUint32_t*)src_ptr, sizeWords );
  // }
   ...
   //	   {
   /*		   oneWord =   (DxUint32_t)src_ptr[3]  |
		              ((DxUint32_t)src_ptr[2] << 8)  |
		              ((DxUint32_t)src_ptr[1] << 16) |
		              ((DxUint32_t)src_ptr[0] << 24 );

		   LLF_PKI_HW_LOAD_VALUE_TO_PKA_MEM( VirtualHwBaseAddr, currAddr, oneWord );
	   ...
	   {
		   src_ptr  = src_ptr  + 4*sizeWords - sizeBytes;
	       oneWord = 0;
		   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)&oneWord,  (DxUint8_t*)src_ptr, sizeBytes % 4 );
	       LLF_PKI_HW_LOAD_VALUE_TO_PKA_MEM( VirtualHwBaseAddr, currAddr, oneWord );
	   }
   } */
...
//#endif /* !defined(_INTERNAL_CRYS_NO_RSA_KG_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_CRT_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_DECCRYPT_SUPPORT) && !defined(_INTERNAL_CRYS_NO_RSA_SIGN_SUPPORT)*/
 ...
 */
                                ...
                                DxUint32_t  VirtualHwBaseAddr  /*in*/ )
 {
   ...
   CRYS_PLAT_SYS_WriteRegister( VirtualHwBaseAddr + LLF_PKI_PKA_OPCODE_REG_ADDR , 0 );
   ...
   {
       /* copy data from PKA register into output buffer */
	   LLF_PKI_HW_READ_BLOCK_FROM_PKA_MEM( VirtualHwBaseAddr, currAddr, (DxUint32_t*)dst_ptr, sizeWords );
   }
   else /* for not aligned data */
   {
	   /* copy full words */
	   for( i = 0; i < sizeBytes/4; i++ )
	   {
		   LLF_PKI_HW_READ_VALUE_FROM_PKA_MEM( VirtualHwBaseAddr , currAddr , oneWord );
	   ...
	   {
	       LLF_PKI_HW_READ_VALUE_FROM_PKA_MEM( VirtualHwBaseAddr , currAddr , oneWord );
		   DX_VOS_FastMemCpy( (DxUint8_t*)dst_ptr, (DxUint8_t*)&oneWord, sizeBytes % 4 );
	   }
   }
 ...
 */
	 ...
	 DxUint32_t  VirtualHwBaseAddr  /*in*/ )
 {
	 ...
	 {
		 /* copy data from PKA register into output buffer */
		 LLF_PKI_HW_LOAD_BLOCK_TO_PKA_MEM( VirtualHwBaseAddr, currAddr, (DxUint32_t*)src_ptr, sizeWords );
		 currAddr = currAddr + 4*sizeWords;
	 }
	 ...
	 else /* for not aligned data */
	 {
		 ...
		 {
			 oneWord = 0;
			 DX_VOS_FastMemCpy( (DxUint8_t*)&oneWord,  (DxUint8_t*)src_ptr, sizeBytes % 4 );
			 LLF_PKI_HW_LOAD_VALUE_TO_PKA_MEM( VirtualHwBaseAddr, currAddr, oneWord );
			 currAddr += 4;
		 }
	 }
 ...
 */
	 ...
	 DxUint32_t VirtualHwBaseAddr  /*in*/ )
 {
	 ...
	 DxUint32_t  size, addr, countTemps;
        ...
        //PRINTF("Entered LLF_PKI_PKA_ClearBlockOfRegs\n");
	 /* FUNCTION LOGIC */
	 ...
	 if( countTemps > 0 )
	 {
        ...
        //PRINTF("Completed LLF_PKI_PKA_ClearBlockOfRegs\n");
...
/*#ifdef CRYS_PKA_SRAM_DATA_MEMORY_MODE
	 DxUint32_t  currWord;
#endif
 ...
 //#ifndef CRYS_PKA_SRAM_DATA_MEMORY_MODE  /* working with HOST RAM */

   //  return CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)(addr + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR), 4*(DxUint16_t)size );
 ...
 *        value, then the function doesn't performs swapping for this pair registers.
 *
