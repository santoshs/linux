...
extern CRYSError_t CRYS_RSA_GenerateVectorInRangeX931(
...
CRYSError_t LLF_PKI_RSA_InitPubKeyDb( CRYSRSAPubKey_t *PubKey_ptr )
{
   ...
   //PRINTF("Entered LLF_PKI_RSA_InitPubKeyDb\n");
   ...
   Error = LLF_PKI_CalcNpAndInitModOp(
                              (DxUint8_t*)PubKey_ptr->n,       /*in*/
                              PubKey_ptr->nSizeInBits,         /*in*/
                              PubKey_ptr->LLF.NP,              /*out*/
                              PKA_CreateNP,                    /*in*/
                              2,   /*rT0*/                     /*in*/
                              3,   /*rT1*/                     /*in*/
                              4,   /*rT2*/                     /*in*/
 ...
 Return:
  //PRINTF("Ended LLF_PKI_RSA_InitPubKeyDb\n");
   return Error;
...
CRYSError_t LLF_PKI_RSA_InitPrivKeyDb( CRYSRSAPrivKey_t *PrivKey_ptr )
{
   ...
   if( PrivKey_ptr->OperationMode == CRYS_RSA_NoCrt )
   {
	   ...
	   /* calculate NP by initialization PKA for modular operations (modulus PrivKey_ptr->n ).
	      ...
	      Our settings for temps: rT0=2, rT1=3, rT2=4 */
		  Error = LLF_PKI_CalcNpAndInitModOp(
		                              (DxUint8_t*)PrivKey_ptr->n,      /*in*/
		                              PrivKey_ptr->nSizeInBits,        /*in*/
		                              PrivKey_ptr->LLF.NonCrt.NP,      /*out*/
		                              PKA_CreateNP,                    /*in*/
		                              2,   /*rT0*/                     /*in*/
		                              3,   /*rT1*/                     /*in*/
		                              4,   /*rT2*/                     /*in*/
   ...
   else
   {
	   ...
	   Error = LLF_PKI_CalcNpAndInitModOp(
                                  (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P, /*in*/
	                              PrivKey_ptr->PriveKeyDb.Crt.PSizeInBits,   /*in*/
	                              PrivKey_ptr->LLF.Crt.PP,                   /*out*/
	                              PKA_CreateNP,                              /*in*/
	                              2,   /*rT0*/                               /*in*/
	                              3,   /*rT1*/                               /*in*/
	                              4,   /*rT2*/                               /*in*/
	   ...
	   Error = LLF_PKI_CalcNpAndInitModOp(
                                  (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q, /*in*/
	                              PrivKey_ptr->PriveKeyDb.Crt.QSizeInBits,   /*in*/
	                              PrivKey_ptr->LLF.Crt.QP,                   /*out*/
	                              PKA_CreateNP,                              /*in*/
	                              2,   /*rT0*/                               /*in*/
	                              3,   /*rT1*/                               /*in*/
	                              4,   /*rT2*/                               /*in*/
                                       ...
                                       CRYS_RSAPrimeData_t *PubData_ptr )
{
  /* error identification */
   ...
   DxUint32_t  modSizeBytes, eSizeInBytes;
   ...
   modSizeBytes = (PubKey_ptr->nSizeInBits + 7) / 8;
   ...
   /* initialize the PKA engine on default mode */
   Error = LLF_PKI_PKA_DefaultInitPKA( PubKey_ptr->nSizeInBits,
                               (PubKey_ptr->nSizeInBits>>5)+1 /*LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS*/, VirtualHwBaseAddr );
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( 2/*dstReg*/, 1/*LenID*/,
                     (DxUint8_t*)PubData_ptr->DataIn, modSizeBytes, VirtualHwBaseAddr );

#ifdef LLF_PKI_PKA_DEBUG
   LLF_PKI_PKA_CopyDataFromPkaReg( (DxUint8_t*)tempRes,  modSizeBytes,
                                   2/*srcReg*/, VirtualHwBaseAddr );

   if( DX_VOS_MemCmp((DxUint8_t*)tempRes, (DxUint8_t*)PubData_ptr->DataIn, modSizeBytes) != 0)
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( 3/*dstReg*/, 1/*LenID*/,
                     (DxUint8_t*)PubKey_ptr->e, eSizeInBytes, VirtualHwBaseAddr );
   ...
   LLF_PKI_PKA_CopyDataFromPkaReg( (DxUint8_t*)PubData_ptr->DataOut, modSizeBytes,
                                        ...
                                        CRYS_RSAPrimeData_t *PrivData_ptr )
{
  ...
  /* error identification */
   ...
   /* initialize the error identifier to success */
   Error = CRYS_OK;
   PRINTF("Entered LLF_PKI_RSA_ExecPrivKeyExp\n");
   ...
   /* initialize the PKA engine on default mode */
   Error = LLF_PKI_PKA_DefaultInitPKA( PrivKey_ptr->nSizeInBits,
                               (PrivKey_ptr->nSizeInBits>>5)+1 /*LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS*/, VirtualHwBaseAddr );
   if( Error != CRYS_OK )     {
       PRINTF("Fauiled in LLF_PKI_PKA_DefaultInitPKA\n");
         goto End;}
   ...
   if( PrivKey_ptr->OperationMode == CRYS_RSA_NoCrt )
   {
       ...
       if( Error != CRYS_OK )     {
        PRINTF("Failed in LLF_PKI_ExecPrivKeyExpNonCrt\n");
         goto End;
       }
...
End:
 ...
 Return:
   PRINTF("Ended LLF_PKI_RSA_ExecPrivKeyExp\n");
   return Error;
...
//					                VirtualHwBaseAddr );
  ...
  //     if( Error != CRYS_OK )
    ...
    //#if !defined RSA_KG_NO_RND
      // if( !Success )
       //{
	//	   Error = CRYS_RSA_GenerateVectorInRangeX931( PubKey_ptr->nSizeInBits / 2, (DxUint8_t*)P_ptr,
          //                                 (DxUint8_t*)PrivKey_ptr->n /*as temp buff*/);
	//	   if( Error != CRYS_OK )
	//	     goto End;

	//	   Error = CRYS_RSA_GenerateVectorInRangeX931( PubKey_ptr->nSizeInBits / 2, (DxUint8_t*)Q_ptr,
	//	                                               (DxUint8_t*)PrivKey_ptr->n /*as temp buff*/ );
	/*	   if( Error != CRYS_OK )
		     goto End;
       }
...
/*	 PrivKey_ptr->PriveKeyDb.NonCrt.dSizeInBits =
     CRYS_COMMON_GetCounterEffectiveSizeInBits((DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d,
                                              (DxUint16_t)(PubKey_ptr->nSizeInBits / 8) );
   }*//* end of non CRT case */
   ...
   /*else
   {
       do
       {
          Error = LLF_PKI_genKeyCrt(PubKey_ptr->e , PubKey_ptr->eSizeInBits,
					               PubKey_ptr->nSizeInBits,
					               0,*/ /*testsCount should be set automatically*/
					               /*&Success,
					               P_ptr,
                                  ...
                                  DxUint32_t  *N_ptr )
{
   ...
   Error = LLF_PKI_RSA_CallRMul( P_ptr , max( PSizeInBits, QSizeInBits ),
                         Q_ptr , N_ptr );
                  ...
                  DxUint32_t *tempBuff_ptr)
{
   ...
   LLF_PKI_PKA_DefaultInitPKA( 32*opSizeWords, LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS, VirtualHwBaseAddr );
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( 2/*dstReg*/, 1/*LenID*/, (DxUint8_t*)A_ptr/*src_ptr*/,
                                   4*ASizeInWords , VirtualHwBaseAddr );

   /* copy divisor into PKA register: B=>r3 */
   LLF_PKI_PKA_CopyDataIntoPkaReg( 3/*dstReg*/, 1/*LenID*/, (DxUint8_t*)B_ptr/*src_ptr*/,
                                   4*BSizeInWords , VirtualHwBaseAddr );
   ...
   {
	   LLF_PKI_PKA_CopyDataFromPkaReg( (DxUint8_t*)divRes_ptr,  4*ASizeInWords,
                                       4/*srcReg*/, VirtualHwBaseAddr );
   }

   if( modRes_ptr != DX_NULL )
   {
	   LLF_PKI_PKA_CopyDataFromPkaReg( (DxUint8_t*)modRes_ptr,  4*BSizeInWords,
                                       2/*srcReg*/, VirtualHwBaseAddr );
   }
 ...
 */
                                   ...
                                   DxUint32_t *Res_ptr )
 {
   ...
   if( 2*ASizeInBits < LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS )

      OpSizeInWords = 2*((LLF_PKI_PKA_MIN_OPERATION_SIZE_BITS + 31))/32;
   ...
   Error = LLF_PKI_PKA_DefaultInitPKA( 32*OpSizeInWords, LLF_PKI_PKA_MAX_REGISTER_SIZE_WORDS, VirtualHwBaseAddr );
   ...
   LLF_PKI_PKA_CopyDataIntoPkaReg( 2/*dstReg*/, 1/*LenID*/, (DxUint8_t*)A_ptr/*src_ptr*/,
                                   2*OpSizeInWords , VirtualHwBaseAddr );
   ...
   LLF_PKI_PKA_CopyDataFromPkaReg( (DxUint8_t*)Res_ptr,  4*OpSizeInWords,
                                       4/*srcReg*/, VirtualHwBaseAddr );
