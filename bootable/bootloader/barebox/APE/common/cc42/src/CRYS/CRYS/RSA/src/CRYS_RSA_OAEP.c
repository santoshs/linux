...

{
  ...
  DxUint8_t  *MaskDB_Ptr=MaskDB;
  DxUint8_t  SeedMask[CRYS_RSA_OAEP_ENCODE_MAX_SEEDMASK_SIZE];  /*buffer in size max hlen*/
 ...
 *--------------------------------------------------------------*/
  ...
  else
  {
		 ...
		 Error=CRYS_HASH_Init(&HashUsercontext,
							   hashFunc);
		 if(Error!=CRYS_OK)
		 {
		   return Error;
		 ...
		 Error=CRYS_HASH_Finish( &HashUsercontext ,
		                          HashResultBuff);
		 if(Error!=CRYS_OK)
		 {
		    return Error;
  ...
  DX_VOS_FastMemCpy(&EM_ptr[HashOutputSize],(DxUint8_t *)HashResultBuff,HashOutputSize);
 ...
 *-----------------------------------------------*/
 ...
 PLAT_LOG_DEV_PRINT_DisplayBuffer(( 20 , (char *)" \n 7. After MGF1 MaskDB = " ,(DxUint8_t *)MaskDB,emLen-HashOutputSize-(VersionConstant-1)));
 /*-----------------------------------------------*
 *                                               *
 ...
 *-----------------------------------------------*/
 ...
 PLAT_LOG_DEV_PRINT_DisplayBuffer(( 20 , (char *)" \n 9.  seedMask = MGF(maskedDB, hLen) = " ,(DxUint8_t *)SeedMask , HashOutputSize));
 ...
 *---------------------------------------------*/
 ...
 /*Clear stack space*/
 DX_VOS_MemSet(MaskDB,0,CRYS_RSA_OAEP_ENCODE_MAX_MASKDB_SIZE);
 DX_VOS_MemSet(SeedMask,0,CRYS_RSA_OAEP_ENCODE_MAX_SEEDMASK_SIZE);
...

{
  ...
  DxUint8_t SeedMask[CRYS_RSA_OAEP_MAX_HLEN];

  DxUint8_t DBMask[CRYS_RSA_OAEP_DECODE_MAX_DBMASK_SIZE];
 ...
 *-------------------------------------------------------*/
 ...
 PLAT_LOG_DEV_PRINT_DisplayBuffer(( 20 , (char *)" \n 4.  seedMask = MGF(maskedDB, hLen) =  ;  " ,(DxUint8_t *)SeedMask , HashOutputSize));
...
Error=CRYS_RSA_OAEPMGF1( HashOutputSize,
 ...
 PLAT_LOG_DEV_PRINT_DisplayBuffer(( 20 , (char *)" \n 6: Let dbMask = MGF(seed, ||EM|| - hLen) = " ,(DxUint8_t *)DBMask , EMSize-HashOutputSize));
 ...
 *-------------------------------------------------------*/
 ...
 else
 {
  ...
  PLAT_LOG_DEV_PRINT_DisplayBuffer(( 20 , (char *)" \n 8: After Hash ; HashResultBuff = " ,(DxUint8_t *)HashResultBuff ,20 ));
 ...
 *------------------------------------------------------*/
 ...
 if(DX_VOS_MemCmp(&EM_ptr[0]+HashOutputSize,(DxUint8_t *)HashResultBuff,HashOutputSize))
     return CRYS_RSA_OAEP_DECODE_ERROR;
/*-----------------------------------------------*
 *                                               *
 * Step 11: Output M.                            *
 ...
 *-----------------------------------------------*/
 ...
 /*Clear Stack space*/
 DX_VOS_MemSet(SeedMask,0,CRYS_RSA_OAEP_MAX_HLEN);
 DX_VOS_MemSet(DBMask,0,CRYS_RSA_OAEP_DECODE_MAX_DBMASK_SIZE);
