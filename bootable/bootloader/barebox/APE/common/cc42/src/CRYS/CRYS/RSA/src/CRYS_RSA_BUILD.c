				                    ...
				                    DxUint16_t ModulusSize )
{
   ...
   /* loading the buffers to little endian */
   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PubKey_ptr->n , Modulus_ptr  , min(ModulusSize , sizeof( PubKey_ptr->n )) );
   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PubKey_ptr->e , Exponent_ptr , min(ExponentSize , sizeof( PubKey_ptr->e )) );

   /* .................. initializing local variables ................... */
   /* ------------------------------------------------------------------- */

   /* .......... initializing the effective counters size in bits .......... */
   ModulusEffectiveSizeInBits =  CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PubKey_ptr->n,ModulusSize );
   ExponentEffectiveSizeInBits = CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PubKey_ptr->e,ExponentSize );
   ...
   /* ...... checking the exponent it must be an integer between 3 and modulus - 1 */

   /* 1. verifying the exponent is less then the modulus */
   CounterCmpResult = CRYS_COMMON_CmpLsbUnsignedCounters(
                         (DxUint8_t*)PubKey_ptr->e, ExponentSize,
                         (DxUint8_t*)PubKey_ptr->n, ModulusSize );
   ...
   /* 2. verifying the exponent is not less then 3 */
				                   ...
				                   DxUint16_t             ModulusSize )
{
   ...
   RETURN_IF_RSA_UNSUPPORTED( UserPrivKey_ptr , PrivExponent_ptr , PrivExponentSize ,
							  ...
							  PrivKey_ptr, Error,Error,Error,Error,Error,Error,Error,Error,Error,Error );
   ...
   /* loading the buffers to little endian */
   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->n ,
                              Modulus_ptr  ,
                              min( ModulusSize , sizeof( PrivKey_ptr->n ) ) );

   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d ,
                              PrivExponent_ptr ,
                              min( PrivExponentSize , sizeof( PrivKey_ptr->PriveKeyDb.NonCrt.d ) ) );
   ...
   ModulusEffectiveSizeInBits =
      CRYS_COMMON_GetCounterEffectiveSizeInBits((DxUint8_t*)PrivKey_ptr->n,ModulusSize );

   PrivExponentEffectiveSizeInBits =
      CRYS_COMMON_GetCounterEffectiveSizeInBits((DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d,PrivExponentSize );
   ...
   /* checking the private exponent: it must be an integer between 3 and modulus - 1 */

   /* 1. verifying the exponent is less then the modulus */
   CounterCmpResult = CRYS_COMMON_CmpLsbUnsignedCounters(
                         (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d, PrivExponentSize,
                         (DxUint8_t*)PrivKey_ptr->n,                   ModulusSize );
   ...
   {
	   /* copy public exponent into the context buffer and calculate it's effective size */
	   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.e ,
								   PubExponent_ptr ,
								   min( PubExponentSize , sizeof( PrivKey_ptr->PriveKeyDb.NonCrt.e ) ) );

	   PubExponentEffectiveSizeInBits =
		   CRYS_COMMON_GetCounterEffectiveSizeInBits((DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.e, PubExponentSize );

	   /* 3. verifying that the exponent is not less than 3 */
	   ...
	   /* 4. verifying that the public exponent is less than the modulus */
	   CounterCmpResult = CRYS_COMMON_CmpLsbUnsignedCounters(
									   (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.e, PubExponentSize,
									   (DxUint8_t*)PrivKey_ptr->n, ModulusSize );
				                      ...
				                      DxUint16_t qInvSize)
{
   ...
   RETURN_IF_RSA_UNSUPPORTED( UserPrivKey_ptr , P_ptr , PSize ,
                              Q_ptr , QSize , dP_ptr ,
                              dPSize , dQ_ptr , dQSize,
                              qInv_ptr , qInvSize , qInv_ptr , qInvSize ,
   ...
   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P ,
                              P_ptr  ,
                              min( PSize , sizeof( PrivKey_ptr->PriveKeyDb.Crt.P ) ) );

   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q ,
                              Q_ptr  ,
                              min( QSize , sizeof( PrivKey_ptr->PriveKeyDb.Crt.Q ) ) );

   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dP ,
                              dP_ptr  ,
                              min( dPSize , sizeof( PrivKey_ptr->PriveKeyDb.Crt.dP ) ) );

   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dQ ,
                              dQ_ptr  ,
                              min( dQSize , sizeof( PrivKey_ptr->PriveKeyDb.Crt.dQ ) ) );

   CRYS_COMMON_ReverseMemcpy( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.qInv ,
                              qInv_ptr  ,
                              min( qInvSize , sizeof( PrivKey_ptr->PriveKeyDb.Crt.qInv ) ) );
   ...
   P_EffectiveSizeInBits    =
      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P,PSize );

   Q_EffectiveSizeInBits    =
      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q,QSize );

   dP_EffectiveSizeInBits   =
      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dP,dPSize );

   dQ_EffectiveSizeInBits   =
      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dQ,dQSize );

   qInv_EffectiveSizeInBits =
      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.qInv,qInvSize );
   ...
   ModulusEffectiveSizeInBits =
      CRYS_COMMON_GetCounterEffectiveSizeInBits((DxUint8_t*)PrivKey_ptr->n,
                                                min(2*PSize, CRYS_RSA_MAXIMUM_MOD_BUFFER_SIZE_IN_WORDS*sizeof(DxUint32_t)) );

   /* .................. checking the validity of the counters ............... */
   /* ------------------------------------------------------------------------ */

   /* ...... checking the first factor size is not 0 in bits */
   ...
   /* ...... checking the second factor size is not 0 in bits */
   ...
   /* ...... checking the size validity of P , Q and the modulus ............ */
   ...
   CounterCmpResult =
      CRYS_COMMON_CmpLsbUnsignedCounters( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dP ,dPSize,
                                          (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P ,PSize );
   ...
   CounterCmpResult =
      CRYS_COMMON_CmpLsbUnsignedCounters( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dQ ,dQSize,
                                          (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q ,QSize );
   ...
   CounterCmpResult =
      CRYS_COMMON_CmpLsbUnsignedCounters( (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.qInv ,qInvSize,
                                          (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P ,PSize );
			                                    ...
			                                    CRYS_RSAConvertKeyToCrtBuffers_t *Buffers_ptr )
{
   ...
   /* Variable indicates a comparing result */
   DxUint32_t CounterCmpResult;
   ...
   /* 1. verifying the exponent is less than the modulus */
   CounterCmpResult = CRYS_COMMON_CmpLsbUnsignedCounters(
                         (DxUint8_t*)e_ptr, ((DxUint16_t)eSizeInBits + 7)/8,
                         (DxUint8_t*)n_ptr, (DxUint16_t)nSizeInBytes );
   ...
   /* 2. verifying the exponent is not less than 3 */
   if( eSizeInBits < 32 && e_ptr[0] < CRYS_RSA_MIN_PUB_EXP_VALUE )

      return  CRYS_RSA_INVALID_EXPONENT_VAL;


   /*  check the size of the private key exponent */
   if(  dSizeInBits > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BITS  )
   ...
   Error = LLF_PKI_RSA_ConvertPrivKeyToCRT( PrivNonCRTKey_ptr,
							n_ptr, nSizeInBits,
							PrivCRTKey_ptr,
							Buffers_ptr);
   ...
   if( Error == CRYS_OK )
   {
	   ...
	   PrivCRTKey_ptr->PSizeInBits  =
	      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivCRTKey_ptr->P, (DxUint16_t)nSizeInBytes );

	   PrivCRTKey_ptr->QSizeInBits =
	      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivCRTKey_ptr->Q, (DxUint16_t)nSizeInBytes );

	   PrivCRTKey_ptr->dPSizeInBits =
	      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivCRTKey_ptr->dP, (DxUint16_t)nSizeInBytes );

	   PrivCRTKey_ptr->dQSizeInBits =
	      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivCRTKey_ptr->dQ, (DxUint16_t)nSizeInBytes );

	   PrivCRTKey_ptr->qInvSizeInBits =
	      CRYS_COMMON_GetCounterEffectiveSizeInBits( (DxUint8_t*)PrivCRTKey_ptr->qInv, (DxUint16_t)nSizeInBytes );
				                  ...
				                  DxUint16_t *ModulusSize_ptr )
{
   ...
   CRYS_COMMON_ReverseMemcpy( Modulus_ptr , (DxUint8_t*)PubKey_ptr->n , nSizeInBytes );
   CRYS_COMMON_LsMsWordsArrayToMsLsBytes( Exponent_ptr , (DxUint8_t*)PubKey_ptr->e , eSizeInBytes );
				                                      ...
				                                      DxUint16_t *ModulusSize_ptr )
{
   ...
   /* calculating the required size in bytes */
   *ModulusSize_ptr = (DxUint16_t)(PubKey_ptr->nSizeInBits / 8);
				                   ...
				                   DxUint16_t            *ModulusSize_ptr )
{
   ...
   RETURN_IF_RSA_UNSUPPORTED( UserPrivKey_ptr , PrivExponent_ptr , PrivExponentSize_ptr ,
                              PubExponent_ptr , PubExponentSize_ptr , Modulus_ptr ,
                              ModulusSize_ptr , nSizeInBytes , dSizeInBytes,
                              eSizeInBytes , PrivKey_ptr , PrivKey_ptr , PrivKey_ptr ,
                              PrivKey_ptr , PrivKey_ptr , PrivKey_ptr ,
                              PrivKey_ptr , PrivKey_ptr , PrivKey_ptr ,
                              PrivKey_ptr , PrivKey_ptr , PrivKey_ptr );
   ...
   /* loading the the buffers */
   CRYS_COMMON_ReverseMemcpy( Modulus_ptr , (DxUint8_t*)PrivKey_ptr->n , nSizeInBytes );

   if( PubExponent_ptr != DX_NULL )
   {
	   CRYS_COMMON_LsMsWordsArrayToMsLsBytes( PubExponent_ptr , (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.e ,
								      eSizeInBytes );
   }

   CRYS_COMMON_ReverseMemcpy( PrivExponent_ptr , (DxUint8_t*)PrivKey_ptr->PriveKeyDb.NonCrt.d , dSizeInBytes );
   ...
   *ModulusSize_ptr  = (DxUint16_t)nSizeInBytes;
   ...
   if( PubExponentSize_ptr != DX_NULL )
   {
	   *PubExponentSize_ptr = (DxUint16_t)eSizeInBytes;
   }

   *PrivExponentSize_ptr = (DxUint16_t)dSizeInBytes;
				                      ...
				                      DxUint16_t *qInvSize_ptr)
{
   ...
   DxUint32_t QSizeInBytes;
   DxUint32_t dPSizeInBytes;
   DxUint32_t dQSizeInBytes;
   DxUint32_t qInvSizeInBytes;
   ...
   RETURN_IF_RSA_UNSUPPORTED( UserPrivKey_ptr , P_ptr , PSize_ptr ,
                              Q_ptr , QSize_ptr , dP_ptr ,
                              dPSize_ptr , dQ_ptr , dQSize_ptr ,
                              qInv_ptr , qInvSize_ptr , qInvSize_ptr , PSizeInBytes ,
                              QSizeInBytes , dPSizeInBytes , dQSizeInBytes ,
                              qInvSizeInBytes , PrivKey_ptr , qInvSize_ptr ,
                              qInvSize_ptr , qInvSize_ptr , qInvSize_ptr );
   ...
   if( PSize_ptr == DX_NULL )
      ...
      return CRYS_RSA_INVALID_CRT_FIRST_FACTOR_SIZE_POINTER_ERROR;
   ...
   if( QSize_ptr == DX_NULL )
      ...
      return CRYS_RSA_INVALID_CRT_SECOND_FACTOR_SIZE_POINTER_ERROR;
   ...
   if( dPSize_ptr == DX_NULL )
      ...
      return CRYS_RSA_INVALID_CRT_FIRST_FACTOR_EXP_SIZE_PTR_ERROR;
   ...
   if( dQSize_ptr == DX_NULL )
      ...
      return CRYS_RSA_INVALID_CRT_SECOND_FACTOR_EXP_SIZE_PTR_ERROR;
   ...
   if( qInvSize_ptr == DX_NULL )
      ...
      return CRYS_RSA_INVALID_CRT_COEFFICIENT_SIZE_PTR_ERROR;
   ...
   if(PrivKey_ptr->OperationMode != CRYS_RSA_Crt)
   {
		return CRYS_RSA_WRONG_PRIVATE_KEY_TYPE;
   }
	...
	{
		return CRYS_RSA_INVALID_CRT_FIRST_FACTOR_SIZE_ERROR;
	}

	if(QSizeInBytes > *QSize_ptr)
	{
		return CRYS_RSA_INVALID_CRT_SECOND_FACTOR_SIZE_ERROR;
	}

	if(dPSizeInBytes > *dPSize_ptr)
	{
		return CRYS_RSA_INVALID_CRT_FIRST_FACTOR_EXP_SIZE_ERROR;
	}

	if(dQSizeInBytes > *dQSize_ptr)
	{
		return CRYS_RSA_INVALID_CRT_SECOND_FACTOR_EXP_SIZE_ERROR;
	}

	if(qInvSizeInBytes > *qInvSize_ptr)
	{
		return CRYS_RSA_INVALID_CRT_COEFFICIENT_SIZE_ERROR;
	}

	/* copy the verctors to the buffers. */
	CRYS_COMMON_ReverseMemcpy( P_ptr,(DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.P, PSizeInBytes );

	CRYS_COMMON_ReverseMemcpy( Q_ptr,(DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.Q, QSizeInBytes );

	CRYS_COMMON_ReverseMemcpy( dP_ptr, (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dP, dPSizeInBytes );

	CRYS_COMMON_ReverseMemcpy( dQ_ptr, (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.dQ, dQSizeInBytes );

	CRYS_COMMON_ReverseMemcpy( qInv_ptr, (DxUint8_t*)PrivKey_ptr->PriveKeyDb.Crt.qInv, qInvSizeInBytes );

	*PSize_ptr = (DxUint16_t)PSizeInBytes;
	*QSize_ptr = (DxUint16_t)QSizeInBytes;
	*dPSize_ptr = (DxUint16_t)dPSizeInBytes;
	*dQSize_ptr = (DxUint16_t)dQSizeInBytes;
	*qInvSize_ptr = (DxUint16_t)qInvSizeInBytes;
				                      ...
				                      DxUint16_t *NSize_ptr)
{
   ...
   RETURN_IF_RSA_UNSUPPORTED( UserPrivKey_ptr , N_ptr , NSize_ptr ,
                              N_ptr , NSize_ptr , N_ptr ,
                              NSize_ptr , N_ptr , PrivKey_ptr ,
                              N_ptr , NSize_ptr , NSize_ptr , NSizeInBytes ,
                              NSizeInBytes , NSizeInBytes , NSizeInBytes ,
                              NSizeInBytes , NSizeInBytes , NSizeInBytes ,
                              NSizeInBytes , NSizeInBytes , NSizeInBytes );
   ...
   /* ...... checking the exponent size ................................ */
	...
	CRYS_COMMON_ReverseMemcpy( N_ptr,(DxUint8_t*)PrivKey_ptr->n, NSizeInBytes );

	*NSize_ptr = (DxUint16_t)NSizeInBytes;
