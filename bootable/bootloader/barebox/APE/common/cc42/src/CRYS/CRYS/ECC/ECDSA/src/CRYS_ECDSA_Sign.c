								           ...
								           CRYS_ECPKI_HASH_OpMode_t       HashMode             /*in*/ )
{
   ...
   PrivKey_ptr = (CRYS_ECPKI_PrivKey_t*)(SignerPrivKey_ptr->PrivKeyDbBuff);
   ...
   { 				
    ...
    DX_VOS_FastMemCpy((DxUint8_t *)&ccmWorkingContext_ptr->ECDSA_SignerPrivKey,(DxUint8_t *)SignerPrivKey_ptr,sizeof(CRYS_ECPKI_UserPrivKey_t));
    ...
    {   
        ...
        case CRYS_ECPKI_AFTER_HASH_SHA1_mode:
			   ...
			   /* Copy the DataIn_ptr to the HashResult */	   			
		   		DX_VOS_FastMemCpy((DxUint8_t *)ccmWorkingContext_ptr->HASH_Result,MessageDataIn_ptr,DataInSize);
		   		break;
                            ...
                            DxUint32_t                     *EphemerKeyData_ptr   /*in*/ ) 
{
   ...
   PrivKey_ptr = (CRYS_ECPKI_PrivKey_t *)ccmWorkingContext_ptr->ECDSA_SignerPrivKey.PrivKeyDbBuff; 
   ...
   /* Temp buffers */
   MessageRepresent_ptr = ccmWorkingContext_ptr->LLF.Tempbuff;
   SignatureC_ptr       = MessageRepresent_ptr + ModSizeInWords + 1;
   SignatureD_ptr       = SignatureC_ptr + ModSizeInWords + 1; /* Max lengths of C in whole words */
   TempBuff_ptr         = SignatureD_ptr + ModSizeInWords + 1 ;
  ...
  /* If the received output buffer is small than 2*OrderSizeInBytes then return an error */
   ...
   if(ccmWorkingContext_ptr->HashMode <= CRYS_ECPKI_HASH_SHA512_mode)
   {
    ...
    DX_VOS_MemSetZero((DxUint8_t *)MessageRepresent_ptr, 4*(ModSizeInWords + 1));

    if(DomainInfo.EC_OrderSizeInBits >= 32*HashSizeWords)  /* Copy HASH_Result into MessageRepresent buffer */
		CRYS_COMMON_ReverseMemcpy((DxUint8_t *)MessageRepresent_ptr, (DxUint8_t *)(ccmWorkingContext_ptr->HASH_Result), 
  		               4*HashSizeWords); 
    else
	{
   	    /* Copy only OrderSizeInBytes bytes of HASH_Result into MessageRepresent */
   		CRYS_COMMON_ReverseMemcpy((DxUint8_t *)MessageRepresent_ptr, (DxUint8_t *)(ccmWorkingContext_ptr->HASH_Result), 
  		                           OrderSizeInBytes); 

        /* Shift right MessageRepresent by redundant bits */
       
        Shift = 8 - (DomainInfo.EC_OrderSizeInBits & 7);
        
        if(Shift != 8)
        {        
	        for(i = 0; i < OrderSizeInWords - 1; i++)
	        {
	        	MessageRepresent_ptr[i] = (MessageRepresent_ptr[i] >> Shift) | (MessageRepresent_ptr[i+1] << (32 - Shift));
	        }        	
   ...
   /*********  CALL LLF ECDSA Sinature function  *****************************************/
	...
	Error =  LLF_ECDSA_SignCalcCall( PrivKey_ptr, MessageRepresent_ptr, HashSizeWords, 
								     /*DX_TRUE*/ IsEphemerKeyInternal,  /*DX_NULL*/ EphemerKeyData_ptr,        						              
	...
	/* If DomainID = WMDRM then output reversed D,C of length ModSizeInBytes each one */
    if(DomainID == CRYS_ECPKI_DomainID_WMDRM10)
	{
		/* Reverse signature to big endian and output it */
		CRYS_COMMON_ReverseMemcpy( SignatureOut_ptr, 
										(DxUint8_t*)SignatureD_ptr, OrderSizeInBytes );
		CRYS_COMMON_ReverseMemcpy( SignatureOut_ptr + OrderSizeInBytes, 
									(DxUint8_t*)SignatureC_ptr, OrderSizeInBytes );
    } 
	else /* For other domains output signature according to ECDSA standard */
	{
        /* Temporary we output reversed C,D strings of length OrderSizeInBytes */
		CRYS_COMMON_ReverseMemcpy( SignatureOut_ptr, 
										(DxUint8_t*)SignatureC_ptr, OrderSizeInBytes );
		CRYS_COMMON_ReverseMemcpy( SignatureOut_ptr + OrderSizeInBytes, 
									(DxUint8_t*)SignatureD_ptr, OrderSizeInBytes );
