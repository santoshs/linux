...
/* define flag for secret key operation with RKEK1 */
#define CRYS_AES_IS_SECRET_KEY_RKEK1			0x1

/* define flag for secret key operation with RKEK2 */
#define CRYS_AES_IS_SECRET_KEY_RKEK2			0x2
...
typedef struct CRYS_AESUserContext_t 
{
   ...
   DxUint32_t   OperationMode;  /* for using in non secured parts of programs */
...
/**
  \brief These macros the user should call when operating the AES in one integrated operation.
   ...
   Macro CRYS_AES is called when the user performs AES operation on any mode.
...
/**
  ...
  @param[in]  DataIn_ptr - A pointer to plain text data to be wrapped
                            NOTE: Overlapping between the data input and data output buffer
                                  is not allowed, except the in place case that is legal . 			
...
#endif /* #ifndef CRYS_AES_CF_H */ 

#endif /* CRYS_AES_CF_VERSION_USED */
