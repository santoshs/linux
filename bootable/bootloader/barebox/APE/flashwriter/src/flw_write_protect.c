...
/**
 * flw_write_protect - write protect task in Flash Writer.
...
RC flw_write_protect( uchar* pbuff, ulong length)
{
	...
	ret = Flash_Access_Write_Protect(start_group, end_group, user_data);
	...
	else if( ret == FLASH_ACCESS_ERR_WRITE_PROTECT )
	{
		ret = FLW_DEVICE_WRITE_PROTECT_ERROR;
	}
