...
static void setup_revision_tag(void)
{
	if (system_rev) {
		...
		printf("setup_revision_tag: 0x%08x\n", system_rev);
...
static void setup_serial_tag(void)
{
	if (system_serial) {
		...
		// printf("setup_serial_tag: 0x%08x\n", system_serial);
...
static void setup_rdinit_tag(struct boot_img_hdr *hdr)
{
if (hdr->ramdisk_size)
	{
		...
		params->u.initrd.start = hdr->ramdisk_addr;
		params->u.initrd.size = hdr->ramdisk_size;
		params = tag_next(params);
	}
}
...
{
	const char *commandline = getenv("bootargs");
	void (*kernel)(int zero, int arch, void *params);

	if(hdr->second_size > 0)
	kernel = (void*)(hdr->second_addr);
	else
	kernel = (void*)(hdr->kernel_addr);

	setup_start_tag();
	setup_memory_tags();
	setup_commandline_tag(commandline, 0);
	setup_rdinit_tag(hdr);
	setup_revision_tag();
	setup_serial_tag();
	setup_end_tag();
