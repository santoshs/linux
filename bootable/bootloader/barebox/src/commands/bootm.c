...
int read_padding(int fd, unsigned pagesize, unsigned itemsize)
{
...
void bootimg_print_image_hdr(struct boot_img_hdr *hdr)
{
	...
	printf ("   rdisk size:    0x%x\n", hdr->ramdisk_size);
	printf ("   rdisk addr:    0x%x\n", hdr->ramdisk_addr);
...
static int do_booti(struct command *cmdtp, int argc, char *argv[])
{
	int	opt;
	
	struct image_handle *os_handle = NULL;
	struct image_handle *initrd_handle = NULL;
	struct image_handler *handler;
	struct image_data data;
	struct boot_img_hdr *hdr = (void*) boothdr;
	char options[53]; /* worst case: whole alphabet with colons */
	int fd;
	memset(&data, 0, sizeof(struct image_data));
	data.verify = 1;
	...
	/* boot header*/
	if (read(fd, hdr, sizeof(struct boot_img_hdr)) < 0) {
	printf("could not read: %s\n", errno_str());
	goto err_out;
	}
	...
	/* second stage */
	if(read_padding(fd, hdr->page_size, hdr->ramdisk_size)) goto err_out;
	if(hdr->second_size > 0)
	{
		if (read(fd, (void*) hdr->second_addr, hdr->second_size) < 0) {
			printf("booti: failed to read second: %s\n", errno_str());
			goto err_out;
		}
	}
	do_booti_linux(hdr);
...
BAREBOX_CMD_HELP_START(booti)
BAREBOX_CMD_HELP_USAGE("booti [-n] image\n")
BAREBOX_CMD_HELP_SHORT("boot an Android OS image.\n")
BAREBOX_CMD_HELP_OPT  ("-n",  "Do not verify the image (speeds up boot process)\n")
BAREBOX_CMD_HELP_END


BAREBOX_CMD_START(booti)
	.cmd		= do_booti,
	.usage		= "boot an Android OS image",
	BAREBOX_CMD_HELP(cmd_booti_help)
BAREBOX_CMD_END
