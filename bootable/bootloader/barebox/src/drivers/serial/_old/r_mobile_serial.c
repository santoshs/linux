...
static void r_mobile_serial_init_port(struct console_device *cdev)
{
	...
	/* SCIFA0 port initialization */
	writel(readl(MPCKCR) & ~(1 << 10), MPCKCR); /* MPCKCSTP */
	writel(readl(MPCKCR) & ~(1 << 9), MPCKCR);  /* MPCKSTP */
	writel(readl(SMSTPCR2) & ~(1 << 4), SMSTPCR2); /* MSTP204 */
	writeb(0x11, GPIO_PORT128CR);	/* SCIFA2 RTS */
	writeb(0xE1, GPIO_PORT129CR);	/* SCIFA2 CTS */
