...
#define MAX98090_EVM                    0      /* enable value */
#define MAX98090_KOTA                   1      /* disable value */
...
typedef struct {
    ...
    u_int volume[MAX98090_VOL_DEV_MAX]; //!< volume level setting.
...
typedef struct {
    ...
    max98090_info info;                     //!< user setting info.
    /* i2c */
    ...
    struct workqueue_struct *irq_workqueue; //!< irq workqueue.
    struct work_struct irq_work;            //!< irq work.
    max98090_switch_data switch_data;       //!< switch data.
    /* callback */
    max98090_callback_func_t *callback;     //!< structure to register a callback function.
    /* log */
...
static int max98090_setup(struct i2c_client *client, max98090_priv *dev);
static int max98090_setup_r8a73734(void);
static int max98090_setup_max98090(void);
static int max98090_setup_max97236(void);
static int max98090_switch_dev_register(max98090_priv *dev);
...
static int max98090_conv_device_info(const u_long device, max98090_info *device_info);
...
static int max98090_set_device_active(const max98090_info new_device);
static int max98090_set_speaker_device(const u_int cur_dev, const u_int new_dev);
static int max98090_set_earpiece_device(const u_int cur_dev, const u_int new_dev);
static int max98090_set_headphone_device(const u_int cur_dev, const u_int new_dev);
static int max98090_set_mic_device(const u_int cur_dev, const u_int new_dev);
static int max98090_set_headset_mic_device(const u_int cur_dev, const u_int new_dev);
...
static int max98090_proc_read(char* page, char** start, off_t offset, int count, int* eof, void* data);
static int max98090_proc_write(struct file* filp, const char* buffer, unsigned long count, void* data);

#ifdef __SOC_CODEC_ADD__
static int max98090_probe(struct snd_soc_codec *codec);
static int max98090_remove(struct snd_soc_codec *codec);
static int max98090_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id);
static int max98090_i2c_remove(struct i2c_client *i2c);
#else
static int max98090_probe(struct i2c_client *i2c, const struct i2c_device_id *id);
...
*/
static max98090_priv *max98090_conf = NULL;

/*!
  @brief max98090 volume table.
*/
static const u_int MAX98090_volume[MAX98090_VOL_DEV_MAX][MAX98090_VOLUMEL_MAX] = {
    /* MAX98090_VOL_DEV_SW */
    { MAX98090_VOLUMEL0, MAX98090_VOLUMEL1, MAX98090_VOLUMEL2, MAX98090_VOLUMEL3, MAX98090_VOLUMEL4, MAX98090_VOLUMEL5 },
    /* MAX98090_VOL_DEV_SPEAKER */
    { 0x18 /* -48dB */,  0x1B /* -36dB */,  0x1F /* -23dB */,  0x24 /* -10dB */,  0x2E /* 2dB  */,   0x3F /* 14db */   },
    /* MAX98090_VOL_DEV_EARPIECE */
    { 0x00 /* -62dB */,  0x03 /* -50dB */,  0x07 /* -35dB */,  0x0C /* -20dB */,  0x12 /* -6dB */,   0x1F /* 8dB */    },
    /* MAX98090_VOL_DEV_HEADPHONES */
    { 0x00 /* -67dB */,  0x03 /* -55dB */,  0x07 /* -40db */,  0x0C /* -25dB */,  0x12 /* -11dB */,  0x1F /* 3dB */    }
};
...
static int max98090_enable_vclk4(void)
{
    ...
    max98090_log_efunc("");
    gpio_request(GPIO_FN_VIO_CKO4, NULL);
...
static int max98090_setup(struct i2c_client *client, max98090_priv *dev)
{
    ...
    if (NULL != dev->log_parent) {
        dev->log_entry = create_proc_entry(MAX98090_LOG_LEVEL, S_IRUGO | S_IWUGO,
                                            dev->log_parent);
    ...
    if (NULL == dev->irq_workqueue) {
        if (NULL == (dev->irq_workqueue = create_singlethread_workqueue("jack_detect"))) {
            ret = -ENOMEM;
    ...
    ret = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS1, &status1);
    if (0 > status1) {
        max98090_conf->board = MAX98090_KOTA;
    } else {
        max98090_conf->board = MAX98090_EVM;
    }
    max98090_log_info("read max97236(0x00) [0x%x]", status1);
...
static int max98090_setup_max98090(void)
{
    ...
    max98090_log_efunc("");
    /* quick setup */
    /* Register 0x04 (System Clock Quick Setup) = 0x80 (Setup Device for Operation with a 26MHz Master Clock (MCLK)) */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_W_SYSTEM_CLOCK, 0x80);
    if (0 != ret) {
        goto err_i2c_write;
    }

    /* Register 0x05 (Sample Rate Quick Setup) = 0x08 (Setup Clocks and Filters for a 48kHz Sample Rate) */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_W_SAMPLE_RATE, 0x08);
    if (0 != ret) {
        goto err_i2c_write;
    }

    /* Register 0x06 (Digital Audio Interface (DAI) Quick Setup) = 0x01 (Setup DAI for I2S Slave Mode Operation) */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_W_INTERFACE, 0x01);
    if (0 != ret) {
        goto err_i2c_write;
    }
    ...
    if (MAX98090_EVM == max98090_conf->board) {
        ...
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_INTERRUPT_MASKS, 0x00);
        if (0 != ret) {
            goto err_i2c_write;
        }
    }
    ...
    ret = max98090_set_volume(MAX98090_DEV_PLAYBACK_SPEAKER, MAX98090_VOLUMEL3);
    if (0 != ret) {
        goto err_set_volume;
    }
    ret = max98090_set_volume(MAX98090_DEV_PLAYBACK_EARPIECE, MAX98090_VOLUMEL3);
    if (0 != ret) {
        goto err_set_volume;
    }
    ret = max98090_set_volume(MAX98090_DEV_PLAYBACK_HEADPHONES, MAX98090_VOLUMEL3);
    if (0 != ret) {
        goto err_set_volume;
    }
    ...
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_MIC2_INPUT_LEVEL, 0x2A);
    if (0 != ret) {
        goto err_i2c_write;
    }
    /* 0x17(LEFT ADC LEVEL)   0x10:AVRG=6dB AVR=3dB */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_LEFT_ADC_LEVEL, 0x10);
    if (0 != ret) {
        goto err_i2c_write;
    }
    /* 0x18(RIGHT ADC LEVEL)  0x10:AVRG=6dB AVR=3dB */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_RIGHT_ADC_LEVEL, 0x10);
    if (0 != ret) {
        goto err_i2c_write;
    }
    /* 0x44(ADC CONTROL) 0x03:OSR128=ADCCLK=64*fS ADCDITHER=Dither enabled ADCHP=ADC is optimized for best performance */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_ADC_CONTROL, 0x03);
    if (0 != ret) {
        goto err_i2c_write;
    }
...
static int max98090_setup_max97236(void)
{
    ...
    ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_ENABLE2, 0x02);
    /* 0x1D(Enable1) 0x80:SHDN */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_ENABLE1, 0x80);

    /* 0x07(Left Volume) 0x39:L/R=independent RMUTEL=Disable LVOL=0db */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_LEFT_VOLUME, 0x39);
    /* 0x08(Right Volume) 0x39:MUTER=Disable RVOL=0db */
    ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_RIGHT_VOLUME, 0x39);
...
static int max98090_enable_interrupt(void)
{
    ...
    if (MAX98090_EVM == max98090_conf->board) {
        ...
        ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_IRQ_MASK1, 0x40);
        if (0 != ret) {
            goto err_i2c_write;
        }
        /* set IRQ Mask2 Enable : HP_L */
        ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_IRQ_MASK2, 0x24);
        if (0 != ret) {
            goto err_i2c_write;
        }
    } else {
...
static int max98090_disable_interrupt(void)
{
    ...
    if (MAX98090_EVM == max98090_conf->board) {
        ...
        ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_IRQ_MASK1, 0x00);
        if (0 != ret) {
            goto err_i2c_write;
        }
        /* set IRQ Mask2 Disable */
        ret = max98090_write(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_IRQ_MASK2, 0x00);
        if (0 != ret) {
            goto err_i2c_write;
        }
    } else {
...
static void max98090_irq_work_func(struct work_struct *unused)
{
    ...
    if (MAX98090_EVM == max98090_conf->board) {
        /* read status */
        res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS1, &status1);
        res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS2, &status2);
        res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS3, &status3);
        res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_ENABLE1, &enable1);
        res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_RW_ENABLE2, &enable2);
        if (0x80 & enable2) {
        ...
        if (0x04 & status2) {
            res = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_PASSIVE_MBH_KEYSCAN_DATA, &press);
            if (0x80 & press) {
    ...
    max98090_log_rfunc("res[%d] state[%d]", res, max98090_conf->switch_data.state);
}
...
static int max98090_conv_device_info(const u_long device, max98090_info *device_info)
{
    ...
    device_info->speaker        = (MAX98090_DEV_PLAYBACK_SPEAKER & device)    ? MAX98090_ENABLE : MAX98090_DISABLE;
    device_info->earpiece       = (MAX98090_DEV_PLAYBACK_EARPIECE & device)   ? MAX98090_ENABLE : MAX98090_DISABLE;
    device_info->headphone      = (MAX98090_DEV_PLAYBACK_HEADPHONES & device) ? MAX98090_ENABLE : MAX98090_DISABLE;
    device_info->mic            = (MAX98090_DEV_CAPTURE_MIC & device)         ? MAX98090_ENABLE : MAX98090_DISABLE;
    device_info->headset_mic    = (MAX98090_DEV_CAPTURE_HEADPHONES & device)  ? MAX98090_ENABLE : MAX98090_DISABLE;
#endif  /* __MAX98090_TODO__ */
...
static int max98090_check_device(const u_long device)
{
    ...
    u_long dev = (MAX98090_DEV_PLAYBACK_SPEAKER |
                ...
                MAX98090_DEV_CAPTURE_HEADPHONES);
...
static int max98090_set_device_active(const max98090_info new_device)
{
    ...
    max98090_log_efunc("new_device.raw_device[%ld]", new_device.raw_device);
...
static int max98090_set_speaker_device(const u_int cur_dev, const u_int new_dev)
{
    ...
    if (cur_dev != new_dev) {
        ...
        if (MAX98090_ENABLE == new_dev) {
            /* speaker on */
            mix_l = 0x02;   /* Select Microphone Input 1 to Left Speaker Mixer */
            mix_r = 0x41;   /* right channel speaker uses left channel speaker clock if both speaker channels are enabled. */
            ...
            if (MAX98090_OE_DAC_ENABLE & oe) {
                ...
                oe |= (MAX98090_OE_SPEAKER_ENABLE | MAX98090_OE_DAC_ENABLE);
            }
        } else {
            /* speaker off */
            mix_l = 0x00;
            mix_r = 0x00;
            if (~(MAX98090_OE_SPEAKER_ENABLE | MAX98090_OE_DAC_ENABLE) & oe) {
                /* other features are enabled. */
                /* io is update none. */
                oe &= ~MAX98090_OE_SPEAKER_ENABLE;
            } else {
...
static int max98090_set_earpiece_device(const u_int cur_dev, const u_int new_dev)
{
    ...
    if (cur_dev != new_dev) {
        ...
        if (MAX98090_ENABLE == new_dev) {
            /* earpiece on */
            mix_l = 0x01;   /* Selects MIC 1 as the Input to the Receiver / Line Out Left Mixer */
            mix_r = 0x82;   /* LINEOUT mode. Selects MIC 2 as the Input to the Line Out Right Mixer */
            if (MAX98090_OE_DAC_ENABLE & oe) {
                /* other features are enabled. */
                /* io is update none. */
                ...
                io |= MAX98090_IO_SDIEN_ON;
                oe |= (MAX98090_OE_EARPIECE_ENABLE | MAX98090_OE_DAC_ENABLE);
            }
        } else {
            /* earpiece off */
            mix_l = 0x00;
            mix_r = 0x00;
            if (~(MAX98090_OE_EARPIECE_ENABLE | MAX98090_OE_DAC_ENABLE) & oe) {
                /* other features are enabled. */
                /* io is update none. */
                oe &= ~MAX98090_OE_EARPIECE_ENABLE;
            } else {
        ...
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090,
                            MAX98090_REG_RW_RCV_OR_LOUTL_MIXER, mix_l);
        if (0 != ret) {
            goto err_i2c_write;
        }
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090,
                            MAX98090_REG_RW_LOUTR_MIXER, mix_r);
        if (0 != ret) {
            goto err_i2c_write;
        }
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090,
    ...
    if (cur_dev != new_dev) {
        ...
        if (MAX98090_ENABLE == new_dev) {
            ...
            if (MAX98090_OE_DAC_ENABLE & oe) {
                ...
                io |= MAX98090_IO_SDIEN_ON;
                oe |= (MAX98090_OE_HEADPHONE_ENABLE | MAX98090_OE_DAC_ENABLE);
            }
        } else {
            /* headphone off */
            if (~(MAX98090_OE_HEADPHONE_ENABLE | MAX98090_OE_DAC_ENABLE) & oe) {
                /* other features are enabled. */
                /* io is update none. */
                oe &= ~MAX98090_OE_HEADPHONE_ENABLE;
            } else {
...
static int max98090_set_mic_device(const u_int cur_dev, const u_int new_dev)
{
    ...
    if (cur_dev != new_dev) {
        ...
        if (MAX98090_ENABLE == new_dev) {
            ...
            ie |= (MAX98090_IE_MIC_ENABLE | MAX98090_IE_ADC_ENABLE);
        } else {
            /* mic off */
            io &= ~(MAX98090_IO_SDOEN_ON | MAX98090_IO_HIZOFF_ON);
            ie &= ~(MAX98090_IE_MIC_ENABLE | MAX98090_IE_ADC_ENABLE);
        ...
        if (MAX98090_EVM == max98090_conf->board) {
            /* handset mic */
            /* 0x15(LEFT ADC MIXER)   0x40:Select IN3-IN4 Differential Input Direct to Left ADC Mixer */
            ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_LEFT_ADC_MIXER, 0x40);
            if (0 != ret) {
                goto err_i2c_write;
            }
            /* 0x16(RIGHT ADC MIXER)  0x40:Select IN3-IN4 Differential Input Direct to Right ADC Mixer */
            ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_RIGHT_ADC_MIXER, 0x40);
            if (0 != ret) {
                goto err_i2c_write;
            }
        } else {
            /* digital mic */
            /* 0x0C(DIGITAL MIC)  0x30:MICCLK=DIGMICCLK=PCLK/8, DIGMICR=Right record channel uses digital microphone input, DIGMICL=Left record channel uses digital microphone input */
            ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_DIGITAL_MIC, 0x30);
            if (0 != ret) {
                goto err_i2c_write;
            }
        }
...
static int max98090_set_headset_mic_device(const u_int cur_dev, const u_int new_dev)
{
    ...
    if (cur_dev != new_dev) {
        ...
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_MIC1_INPUT_LEVEL, 0x4A);
        if (0 != ret) {
            goto err_i2c_write;
        }
        /* 0x15(LEFT ADC MIXER)   0x04:Select Microphone Input 1 to Left ADC Mixer */
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_RW_LEFT_ADC_MIXER, 0x04);
        if (0 != ret) {
            goto err_i2c_write;
        }
        ret = max98090_write(MAX98090_AUDIO_IC_MAX98090,
...
static int max98090_dump_max98090_registers(void)
{
    ...
    for (i = 0; i < MAX98090_REG_ID_MAX; i++) {
        ...
        case MAX98090_REG_W_ANALOG_LINE_LOOP:
            max98090_log_debug("addr[0x%02X] is write-only register", MAX98090_reg_addr[i]);
            break;
        default:
            ret = max98090_read(MAX98090_AUDIO_IC_MAX98090, i, &val);
            if (0 != ret) {
                goto err_i2c_read;
            }
            max98090_log_debug("ret[%d] addr[0x%02X] val[0x%02X]", ret, MAX98090_reg_addr[i], val);
            break;
        }
    }
...
static int max98090_dump_max97236_registers(void)
{
    ...
    for (i = 0; i < MAX97236_REG_ID_MAX; i++) {
        ...
        max98090_log_debug("ret[%d] addr[0x%02X] val[0x%02X]", ret, MAX97236_reg_addr[i], val);
    }
...
static void max98090_dump_max98090_priv(void)
{
    ...
    if (max98090_conf) {
        ...
        max98090_log_debug("max98090_conf->board[%d]", max98090_conf->board);
        max98090_log_debug("max98090_conf->info.raw_device[0x%0lx]", max98090_conf->info.raw_device);
        max98090_log_debug("max98090_conf->info.speaker[%d]", max98090_conf->info.speaker);
        max98090_log_debug("max98090_conf->info.earpiece[%d]", max98090_conf->info.earpiece);
        max98090_log_debug("max98090_conf->info.headphone[%d]", max98090_conf->info.headphone);
        max98090_log_debug("max98090_conf->info.mic[%d]", max98090_conf->info.mic);
        max98090_log_debug("max98090_conf->info.headset_mic[%d]", max98090_conf->info.headset_mic);
        for (i = 0; i < MAX98090_VOL_DEV_MAX; i++) {
            max98090_log_debug("max98090_conf->info.volume[%d]=[%d]", i, max98090_conf->info.volume[i]);
        }
        max98090_log_debug("max98090_conf->info.mute[%d]", max98090_conf->info.mute);
        max98090_log_debug("max98090_conf->info.speaker_amp[%d]", max98090_conf->info.speaker_amp);
        max98090_log_debug("max98090_conf->client_max97236[0x%p]", max98090_conf->client_max97236);
        max98090_log_debug("max98090_conf->client_max98090[0x%p]", max98090_conf->client_max98090);
        max98090_log_debug("max98090_conf->irq[%d]", max98090_conf->irq);
        max98090_log_debug("max98090_conf->callback[0x%p]", max98090_conf->callback);
        max98090_log_debug("max98090_conf->irq_workqueue[0x%p]", max98090_conf->irq_workqueue);
        max98090_log_debug("max98090_conf->irq_work[0x%p]", &max98090_conf->irq_work);
        max98090_log_debug("max98090_conf->log_entry[0x%p]", max98090_conf->log_entry);
        max98090_log_debug("max98090_conf->log_parent[0x%p]", max98090_conf->log_parent);
        max98090_log_debug("-------------------------------------");
    } else {
        max98090_log_debug("-------------------------------------");
...
{
    int ret = 0;
    max98090_info new_device = max98090_conf->info;
...
int max98090_get_device(u_long *device)
{
    ...
    if (NULL == device) {
        ret = -EINVAL;
        max98090_log_err("parameter error. device is null. ret[%d]", ret);
        return ret;
    }
...
int max98090_set_volume(const u_long device, const u_int volume)
{
    ...
    if (MAX98090_DEV_PLAYBACK_SPEAKER & device) {
        ...
        if (0 != ret) {
            ...
            max98090_conf->info.volume[MAX98090_VOL_DEV_SPEAKER] = volume;
        }
    }
    ...
    if (MAX98090_DEV_PLAYBACK_EARPIECE & device) {
        ...
        if (0 != ret) {
            ...
            max98090_conf->info.volume[MAX98090_VOL_DEV_EARPIECE] = volume;
        }
    }
    ...
    if (MAX98090_DEV_PLAYBACK_HEADPHONES & device) {
        ...
        if (0 != ret) {
            ...
            max98090_conf->info.volume[MAX98090_VOL_DEV_HEADPHONES] = volume;
        }
    }
...
int max98090_get_volume(const u_long device, u_int *volume)
{
    ...
    if (NULL == volume) {
       ret = -EINVAL;
        max98090_log_err("parameter error. volume is null. ret[%d]", ret);
        return ret;
    }

    /* get value */
    if (MAX98090_DEV_PLAYBACK_SPEAKER & device) {
        *volume = max98090_conf->info.volume[MAX98090_VOL_DEV_SPEAKER];
    } else if (MAX98090_DEV_PLAYBACK_EARPIECE & device) {
        *volume = max98090_conf->info.volume[MAX98090_VOL_DEV_EARPIECE];
    } else if (MAX98090_DEV_PLAYBACK_HEADPHONES & device) {
        *volume = max98090_conf->info.volume[MAX98090_VOL_DEV_HEADPHONES];
    } else {
        /* nothing to do. */
    }
...
int max98090_get_mute(u_int *mute)
{
    ...
    if (NULL == mute) {
        ret = -EINVAL;
        max98090_log_err("parameter error. mute is null. ret[%d]", ret);
        return ret;
    }
...
int max98090_get_speaker_amp(u_int *value)
{
    ...
    if (NULL == value) {
        ret = -EINVAL;
        max98090_log_err("parameter error. value is null. ret[%d]", ret);
        return ret;
    }
    /* get value */
...
int max98090_get_status(u_long *irq_status)
{
    ...
    if (NULL == irq_status) {
        ret = -EINVAL;
        max98090_log_err("parameter error. irq_status is null. ret[%d]", ret);
        return ret;
    }

    if (MAX98090_EVM == max98090_conf->board) {
        ret = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS1, &status1);
        ret = max98090_read(MAX98090_AUDIO_IC_MAX97236, MAX97236_REG_OR_STATUS2, &status2);
        *irq_status |= (status1 << 8);
        *irq_status |= status2;
    } else {
        ret = max98090_read(MAX98090_AUDIO_IC_MAX98090, MAX98090_REG_R_DEVICE_STATUS, &status1);
        *irq_status = status1;
    }
...
int max98090_register_callback_func(max98090_callback_func_t *callback_)
{
    ...
    if (NULL == callback_) {
        ret = -EINVAL;
        max98090_log_err("parameter error. callback_ is null. ret[%d]", ret);
        return ret;
    }

    if (NULL == callback_->irq_func) {
        ret = -EINVAL;
        max98090_log_err("parameter error. callback_->irq_func is null. ret[%d]", ret);
        return ret;
    }
...
if (reg & (1 << 17)) {
    ...
    if (500 == i) {
        printk(KERN_WARNING "iowrite32((1 << 17), SYSC_SWUCR) Wake up error \n");
    }
}
#endif  /* __MAX98090_TODO_POWER__ */
    ...
    switch (audio_ic) {
    case MAX98090_AUDIO_IC_MAX98090:
        *value = i2c_smbus_read_byte_data(max98090_conf->client_max98090,
                                        MAX98090_reg_addr[addr_id]);
        max98090_log_info("MAX98090 addr[0x%02X] value[0x%02X]", MAX98090_reg_addr[addr_id], *value);
        break;
    case MAX98090_AUDIO_IC_MAX97236:
        if (MAX98090_EVM == max98090_conf->board) {
            *value = i2c_smbus_read_byte_data(max98090_conf->client_max97236,
                                            MAX97236_reg_addr[addr_id]);
            max98090_log_info("MAX97236 addr[0x%02X] value[0x%02X]", MAX97236_reg_addr[addr_id], *value);
        } else {
            /* nothing to do. */
        }
        break;
...
if (reg & (1 << 17)) {
    ...
    switch (audio_ic) {
    case MAX98090_AUDIO_IC_MAX98090:
        max98090_log_info("addr[0x%02X] value[0x%02X]", MAX98090_reg_addr[addr_id], value);
        ret = i2c_smbus_write_byte_data(max98090_conf->client_max98090,
                                    MAX98090_reg_addr[addr_id], value);
        break;

    case MAX98090_AUDIO_IC_MAX97236:
        if (MAX98090_EVM == max98090_conf->board) {
            max98090_log_info("addr[0x%02X] value[0x%02X]", MAX97236_reg_addr[addr_id], value);
            ret = i2c_smbus_write_byte_data(max98090_conf->client_max97236,
...
static int max98090_proc_write(struct file* filp, const char* buffer, unsigned long count, void* data)
{
    ...
    max98090_log_efunc("filp[%p] buffer[%p] count[%ld] data[%p]", filp, buffer, count, data);
    in = simple_strtoull(buffer, NULL, 0);
...
{
    int ret;
    max98090_priv *max98090 = snd_soc_codec_get_drvdata(codec);
    ...
    ret = snd_soc_add_controls(codec, max98090_controls, max98090_array_size);
    if (ret)
...
static void max98090_set_soc_controls(struct snd_kcontrol_new *controls, u_int array_size)
{
...
#ifdef __SOC_CODEC_ADD__
static int max98090_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
#else
static int max98090_probe(struct i2c_client *client, const struct i2c_device_id *id)
#endif  /* __SOC_CODEC_ADD__ */
{
    int ret = 0;
    max98090_priv *dev = NULL;

    max98090_log_efunc("client[0x%p] id->driver_data[%ld]", client, id->driver_data);
    if (NULL == max98090_conf) {
        dev = kzalloc(sizeof(max98090_priv), GFP_KERNEL);
        if (NULL == dev) {
            ret = -ENOMEM;
            max98090_log_err("Could not allocate master. ret[%d]", ret);
            return ret;
        }
        max98090_conf = dev;
    } else {
    ...
    max98090_sysc_Base = (u_long)ioremap_nocache(SYSC_PHY_BASE, SYSC_REG_MAX);
    if (0 >= max98090_sysc_Base) {
        printk(KERN_WARNING "max98090_probe() SYSC ioremap failed error\n");
        return 0;
    }
#endif /* __MAX98090_TODO_POWER__ */
    ...
    default:
        ret = -EINVAL;
        max98090_log_err("invalid I2C address specified. ret[%d] addr[%02X]",
                        ret, client->addr);
        goto err_slave_addr;
    }
    ...
    if (MAX98090_SLAVE_ADDR == client->addr) {
        /* printk(KERN_INFO "%s : call snd_soc_register_codec()\n", __func__); */
...
#endif  /* __SOC_CODEC_ADD__ */
{
    ...
    max98090_log_efunc("");
    if (((MAX98090_EVM == max98090_conf->board) && (NULL == max98090_conf->client_max98090) && (NULL == max98090_conf->client_max97236)) ||
        ((MAX98090_KOTA == max98090_conf->board) && (NULL == max98090_conf->client_max98090))) {
    ...
    if (max98090_conf->log_entry) {
        remove_proc_entry(MAX98090_LOG_LEVEL, max98090_conf->log_parent);
    }
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("CP1+ onchip Audio LSI Driver");
MODULE_AUTHOR("Xxxx Yyyy <xxxx.yyyy@renesas.com>");
