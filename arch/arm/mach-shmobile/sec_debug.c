...
#define S5P_INFORM1 (S5P_INFORM0        + 0x00000004)
#define S5P_INFORM2 (S5P_INFORM1        + 0x00000004)
#define S5P_INFORM3 (S5P_INFORM2        + 0x00000004)
#define S5P_INFORM4 (S5P_INFORM3        + 0x00000004)
#define S5P_INFORM5 (S5P_INFORM4        + 0x00000004)
#define S5P_INFORM6 (S5P_INFORM5        + 0x00000004)
#define S5P_INFORM7 (S5P_INFORM6        + 0x00000004)
#else
...
#if defined(CONFIG_ARCH_SHMOBILE)
	.priority = -1 /* to put panic handler on last list */
#endif
};
...
static ssize_t sec_write_cp_crash_reason(struct file *file, const char __user *buf, unsigned long count, void *data)
{
	...
	if(!access_ok(VERIFY_READ, buf, count))
		return -EFAULT;
	if(count > sec_cp_crash_reason_inform.size)
		return -EFAULT;
...
static __exit void sec_debug_cp_crash_reason_exit(void)
{
	remove_proc_entry("sec_cp_crash",NULL);
	sec_cp_crash_proc = NULL;
}
