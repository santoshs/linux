...
#if defined(CONFIG_SEC_DEBUG_INFORM)
	sec_debug_inform_magic_write(magic);
#else
...
#if defined(CONFIG_SEC_DEBUG_INFORM)
	sec_debug_inform_write(type, 3);
	sec_debug_inform_write(type, 4);
	sec_debug_inform_write(type, 6);
#else
...
#if 0 //remove unhelpful debug log
	show_state();
#endif
...
{
	static unsigned long vol_down_jiffies = 0;
	...
	if (code == KEY_VOLUMEDOWN)
		vol_down_jiffies = value ? jiffies : 0;
	...
	/* press vol down first and then vol up */
	if (vol_down_jiffies && vol_up_jiffies
	    && time_after(vol_up_jiffies, vol_down_jiffies)) {
		if (jiffies_to_msecs(vol_up_jiffies - vol_down_jiffies) >
		    SEC_TIME_FORCE_CLOSE_DURATION) {
			pr_err("%s: %u msec after vol down\n", __func__,
			       jiffies_to_msecs(vol_up_jiffies -
						vol_down_jiffies));
			panic("Crash Key");
		}
	}
				...
				size_t count, loff_t * offs)
{
	...
	if (strncmp(buf, "dump_user_fault", 15) == 0)
		sec_user_fault_dump();
	else if(!strncmp(buf, "CP Crash", count))
		panic(buf);
...
{
#if defined(CONFIG_ARCH_EXYNOS)
	if (reserve_bootmem(S5P_PA_SDRAM, SZ_4K, BOOTMEM_EXCLUSIVE)) {
...
#elif defined(CONFIG_ARCH_SHMOBILE)
	pr_info("%s: skip reserving magic code area\n", __func__);
#endif
	return 0;
}
