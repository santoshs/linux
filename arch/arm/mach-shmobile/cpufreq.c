...
{
#ifdef CONFIG_U2_ES2
	if (freq == main_freqtbl[FREQ_LEV_HIGH].frequency)
		return DIV1_1;
#endif /* CONFIG_U2_ES2 */
...
struct clk_rate *__clk_get_rate(enum clock_mode clk_state,
	enum sgx_flg_type sgx_type, unsigned int freq)
{
	...
	if (MODE_SUSPEND == clk_state)
		...
		freq_mode = clk_state * sgx_num * lv_num +
			sgx_type * lv_num + level;
#else
	if (MODE_SUSPEND == clk_state)
		freq_mode = MODE_8;
	else if (MODE_EARLY_SUSPEND == clk_state)
...
int stop_cpufreq(void)
{
	...
	if (0 == atomic_read(&cpu_info.cpufreq.usage_count)) {
		...
		if (ret < 0) {
			...
			__notify_frequency_change(freq_old, freq_new,
				num_possible_cpus(),
				CPUFREQ_POSTCHANGE);
			clk_div = __clk_get_rate(cpu_info.clk_state,
				cpu_info.sgx_flg, freq_new);
		}
	}
	atomic_inc(&cpu_info.cpufreq.usage_count);
	spin_unlock(&cpu_info.lock);
		...
		if (__to_freq_level(cpu_info.freq) == FREQ_LEV_MIN) {
			...
			if (ret < 0) {
				...
				__notify_frequency_change(freq_old, freq_new,
					...
					CPUFREQ_POSTCHANGE);
				clk_div = __clk_get_rate(cpu_info.clk_state,
					cpu_info.sgx_flg, cpu_info.freq);
			}
		}
	}
...
int suspend_cpufreq(void)
{
	...
	if (NULL != cpu_info.curtbl_ex1) {
		...
		__notify_frequency_change(freq_old, freq_new,
			num_possible_cpus(), CPUFREQ_POSTCHANGE);
	}
	...
	unsigned int target_freq, unsigned int relation)
{
	...
	if (true == cpu_info.cpufreq.used) {
		freqs.new = main_freqtbl[FREQ_LEV_MAX].frequency;
		__notify_frequency_change(freqs.old, freqs.new,
			num_possible_cpus(), CPUFREQ_POSTCHANGE);
		spin_unlock(&cpu_info.lock);
		return ret;
	}

	if (cpu_info.without_dvfs_flg || cpu_info.suspend_mode) {
		spin_unlock(&cpu_info.lock);
...
next:
	...
	if (freq == cpu_info.freq) {
		__notify_frequency_change(freqs.old, freq, num_possible_cpus(),
			CPUFREQ_POSTCHANGE);
		clk_div = __clk_get_rate(cpu_info.clk_state, cpu_info.sgx_flg,
			cpu_info.freq);
		if (NULL == clk_div) {
...
#endif
	/* change SYS-CPU frequency to MAX. */
	ret = shmobile_set_frequency(freq);
	if (ret < 0) {
