...
static netdev_tx_t mhi_xmit_start(
	struct sk_buff   	*skb, 
	struct net_device	*dev )
{
	...
	if (likely(req->status == 0)) {
		spin_lock_irqsave(&mhi->rx_lock, flags);
		{
			...
			if (!skb) {
				...
				skb = mhi->rx_skb = netdev_alloc_skb(dev, MHI_HEADER_MAX);
				if (unlikely(!skb))
					goto error;
				
				offset = MIN(req->actual_length, MHI_HEADER_MAX);
				memcpy(skb_put(skb,offset), page_address(page), offset);
			} 
			else {
				offset = 0;
			}

			skb_add_rx_frag(skb, skb_shinfo(skb)->nr_frags, page, offset, req->actual_length-offset);
