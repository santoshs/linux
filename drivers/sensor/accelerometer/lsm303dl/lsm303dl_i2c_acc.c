...
static int lsm303dl_acc_set_sensitivity(u8 val)
{
	...
	lsm303dl_acc_info->sensitivity = val;
		...
		if (ret < 0) {
			lsm303dl_log("Cannot configure for INT1_THS_A reg\n");
			return ret;
		}
	#endif
...
int lsm303dl_acc_set_odr(u32 val)
{
	...
	/*Interrupt mechanism is used*/
		...
		if (ACC_ENABLE == activation_flg) {
			...
			if (val > LSM303DL_POLL_THR) {
				...
				if (ret < 0) {
					lsm303dl_log("Fail to power down Accel\n");
					return -EIO;
				}
		...
		if (ACC_ENABLE == activation_flg) {
			if (lsm303dl_acc_info->delay > LSM303DL_POLL_THR) {
				if (val <= LSM303DL_POLL_THR) {
					...
					if (ret < 0) {
						lsm303dl_log("Fail to power on Accel\n");
						return -EIO;
					}
				}
			}
	...
	if ((lsm303dl_acc_info->delay > 100) && \
				(lsm303dl_acc_info->delay < 1000)) {
