...
#include <linux/gpio.h>		/* For GPIO --> new */
#ifdef PMIC_ENABLE
/*#include <linux/pmic/pmic.h>*/	/* For power management */
#endif
...
static int l3gd20_set_high_pass_filter_cutoff_frequency(unsigned char hpf_cf_freq);
...
static int l3gd20_i2c_probe(struct i2c_client *client,  const struct i2c_device_id *devid)
{
	...
	INIT_WORK(&l3gd20_info->irq_work_func, l3gd20_irq_work_func);
	...
	/*Enable I2_DRDY interrupt*/
	ret = l3gd20_register_update(L3GD20_CTRL_REG3, 0x0F, 0x0F);
	if (ret != 0) {
		l3gd20_log("Can not enable I2_DRDY interrupt !\n");
		goto err_register_interrupt;
	}
#endif
...
err_hw_initialize:
	...
	free_irq(l3gd20_client->irq, l3gd20_info);
#endif
...
static long l3gd20_misc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	/* command is L3GD20_IOCTL_SET_DELAY */
		...
		break;
	case L3GD20_IOCTL_NV_DATA_ADDRESS:
	/* command is L3GD20_IOCTL_NV_DATA_ADDRESS */
...
static int l3gd20_set_output_data_rate(unsigned long poll_interval)
{
	...
	if (ENABLE == drv_state) {
		...
		/* old polling interval > Power threshold */
		if (l3gd20_info->poll_cycle > POWER_POLLING_THRES) {
			/* Cancel high resolution timer */
			hrtimer_cancel(&l3gd20_info->timer);
		} else if (l3gd20_info->poll_cycle > MAX_ODR) {
			/* Cancel high resolution timer */
			hrtimer_cancel(&l3gd20_info->timer);
			/* Disable output data & Power off */
			ret = l3gd20_register_update(L3GD20_CTRL_REG1, 0x0F, 0);
...
{
	void *ptr;
	unsigned char buf[L3GD20_SETTING_MAX];

	ptr = ioremap(addr, sizeof(buf));
...
static int __init l3gd20_init(void)
{
	...
	if (ret < 0) {
		l3gd20_log(" GPIO direction setup error\n");
		return -ENOTSUPP;
	}
#endif
#ifdef PMIC_ENABLE
	...
	/* Get I2C adapter */
	l3gd20_adapter = i2c_get_adapter(L3GD20_I2C_BUS_NUMBER/*lookup key*/);
	if (NULL == l3gd20_adapter) {
		l3gd20_log(" Get I2C adapter failed\n");
	...
	/* Add I2C module */
	i2c_put_adapter(l3gd20_adapter);
