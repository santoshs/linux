...
/* For Debugging */
#define atsn01p_dbgmsg(str, args...) pr_debug("[ASP01] %s: " str, __func__, ##args)
#define atsn01p_infomsg(str, args...) pr_info("[ASP01] %s: " str, __func__, ##args)
#undef GRIP_DEBUG
#define USE_EEPROM
...
static int atsn01p_open_calibration(struct atsn01p_data *data)
{
	...
	if (IS_ERR(cal_filp)) {
		...
		if (err != -ENOENT)
			...
			pr_info("[ASP01] %s: need calibration\n", __func__);
...
set_init_code:
	...
	if (!count) {
		...
		pr_info("[ASP01] %s: now use eeprom\n", __func__);
#endif
#ifdef GRIP_DEBUG
	...
	for (i = 0; i < CAL_DATA_NUM; i++) {
		...
		pr_info("[ASP01] %s: verify reg(0x%x) = 0x%x\n", __func__,
...
static int atsn01p_grip_disable(struct atsn01p_data *data)
{
	...
	do {
		...
		pr_info("[ASP01] %s: grip=%d, count=%d\n",
			__func__, grip, count);
	} while (!grip && count--);
...
static void atsn01p_work_func(struct work_struct *work)
{
	...
	if (!g_atsn01p->skip_data) {
		pr_info("[ASP01] %s: touch INT : %d!!\n", __func__, touch);
		input_report_rel(g_atsn01p->input, REL_MISC, touch + 1);
		input_sync(g_atsn01p->input);
	} else
		pr_info("[ASP01] %s: skipped input report jig_on: %d, skip cmd: %d\n",
					...
					const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		pr_info("[ASP01] %s: invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
	...
	mutex_unlock(&g_atsn01p->data_mutex);
		...
		for (i = 0; i < SET_REG_NUM; i++) {
			i2c_smbus_read_i2c_block_data(
				...
				sizeof(reg), &reg);
			pr_info("[ASP01] %s: reg(0x%x) = 0x%x, default 0x%x\n",
				__func__,
		...
		for (i = 0; i < CAL_DATA_NUM; i++) {
			i2c_smbus_read_i2c_block_data(
				...
				sizeof(reg), &reg);
			pr_info("[ASP01] %s: verify reg(0x%x) = 0x%x, data: 0x%x\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 4; i++) {
		...
		for (j = REG_CR_CNT; j < (REG_CS_PERH + 1); j++) {
			...
			pr_info("[ASP01] %s: SEL %d, reg (0x%x) = 0x%x",
				__func__, i, j,
				crcs[(12 * i) + j - REG_CR_CNT]);
		}
	}
...
static int atsn01p_apply_hw_dep_set(struct atsn01p_data *data)
{
	...
	for (i = 0; i < CAL_DATA_NUM; i++) {
		...
		pr_info("[ASP01] %s: default_mfm[%d] = %x\n",
	...
	for (i = 0; i < SET_REG_NUM; i++) {
		if (i < SET_HW_CON1)
			...
			if (reg_prom_en1 != 0xaa)
				...
				init_reg[i][CMD] = reg;
				pr_info("[ASP01] %s: skip HW_CONs, reset value = %x\n",
					__func__, reg);
			}
		}
	}
