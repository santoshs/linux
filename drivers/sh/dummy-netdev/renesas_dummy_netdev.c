...
static int dummy_ioctl(struct net_device *device, struct ifreq *ifr, int cmd)
{
	...
	printk(KERN_NOTICE "**********  dummy_ioctl ***************");
	...
	case SIOCPNGAUTOCONF:{
		...
		printk(KERN_NOTICE "**********  dummy_ioctl: SIOCPNGAUTOCONF ***************");
		req->ifr_phonet_autoconf.device = PN_DEV_HOST;

		iRet = phonet_route_add(device,
			SMC_CONTROL_PHONET_ROUTE_DEVICE);

		if (iRet)
			printk(KERN_NOTICE "dummy_ioctl: SIOCPNGAUTOCONF failed (%d)", iRet);
