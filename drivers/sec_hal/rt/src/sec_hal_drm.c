...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_enter(uint32_t thr_id, uint32_t *session_id)
{
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_set_entit_key(uint32_t session_id, uint32_t key_len,
		uint8_t *key)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_derive_cw(uint32_t session_id, uint32_t ecm_len,
		uint8_t *ecm, uint32_t *flags)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
	uint32_t ssa_disp_st = 0x00;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_decrypt_video(uint32_t session_id, uint8_t *iv_in,
		uint32_t iv_len, uint32_t input_len, uint32_t input_phys_addr,
		uint32_t output_phys_addr, uint32_t output_offset,
		uint32_t *output_len, uint8_t *iv_out)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
	uint32_t ssa_disp_st = 0x00;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_decrypt_audio(uint32_t session_id, uint8_t *iv_in,
		uint32_t iv_len, uint32_t input_len,
		uint32_t input_phys_addr, uint32_t output_phys_addr,
		uint32_t *output_len, uint8_t *iv_out)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_wrap_keybox(uint8_t *keybox, uint32_t keybox_size,
		uint8_t *wrapped_keybox, uint32_t *wrapped_keybox_size,
		uint8_t *transport_key, uint32_t transport_key_size)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
	uint32_t ssa_disp_st = 0x00;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_install_keybox(uint8_t *keybox, uint32_t keybox_size)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
	uint32_t ssa_disp_st = 0x00;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_get_random(uint32_t size, uint8_t *random)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
	uint32_t ssa_disp_st = 0x00;
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_get_device_id(uint32_t id_len, uint8_t *id,
		uint32_t *out_size)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
...
**                      failure otherwise.
** *************************************************************************/
uint32_t sec_hal_drm_get_key_data(uint32_t key_data_len, uint8_t *key_data,
		uint32_t *out_size)
{
	uint32_t sec_hal_st = SEC_HAL_RES_OK;
	uint32_t sec_msg_st[2] = {SEC_MSG_STATUS_OK};
...
static inline
int has_drm_session(struct _drm_data *data, uint32_t id)
{
...
static inline
uint32_t _drm_enter(struct _drm_data *data, sd_ioctl_params_t *params)
{
	uint32_t id, ret;
...
static inline
uint32_t _drm_exit(struct _drm_data *data, sd_ioctl_params_t *params)
{
	uint32_t id, ret;
...
_drm_set_entit_key(struct _drm_data *data, sd_ioctl_params_t *params)
{
	...
	if (has_drm_session(data, id)
		...
		if (copy_from_user(key, (const void __user *)params->param1,
					key_len))
			ret = SEC_HAL_RES_FAIL;
		else
...
static inline
uint32_t _drm_derive_cw(struct _drm_data *data, sd_ioctl_params_t *params)
{
	...
	if (has_drm_session(data, id)
		...
		if (copy_from_user(ecm, (const void __user *)params->param1,
					ecm_len))
			ret = SEC_HAL_RES_FAIL;
		else {
...
#define IV_SZ 16
static inline
uint32_t _drm_decrypt_video(struct _drm_data *data,
		sd_ioctl_params_t *params, struct platform_device *pdev)
{
	...
	id = params->param0;
	if (has_drm_session(data, id)) {
		iv_usr = (void *)params->param1;
		inl = params->param3;

		if (iv_usr && copy_from_user(iv, (const void __user *)iv_usr,
					IV_SZ))
		...
		if (copy_from_user(input, (const void __user *)params->param2,
					inl)) {
			kfree(input);
			return SEC_HAL_RES_FAIL;
		}
		...
		ret = sec_hal_drm_decrypt_video(id, iv, iv_usr ? IV_SZ : 0,
				inl, phys_in, params->param4,
				params->param5, &outl, iv+IV_SZ);
...
static inline
uint32_t _drm_decrypt_audio(struct _drm_data *data,
		sd_ioctl_params_t *params, struct platform_device *pdev)
{
	...
	if (has_drm_session(data, id)) {
		...
		dma_unmap_single(&pdev->dev, phys_in, pad_size(inl), DMA_TO_DEVICE);
		dma_unmap_single(&pdev->dev, phys_out, pad_size(inl), DMA_FROM_DEVICE);
		kfree(output);
		kfree(input);

		if (SEC_HAL_RES_OK == ret) {
			if ((iv_usr && copy_to_user((void __user *)iv_usr, iv+IV_SZ, IV_SZ))
...
static inline
uint32_t _drm_wrap_keybox(sd_ioctl_params_t *params)
{
	...
	kbox_usr = (void *)params->param1;
	kbox_size = params->param2;
	wr_kbox_usr = (void *)params->param3;
	trs_key_usr = (void *)params->param5;
	...
	if (copy_from_user(kbox, (const void __user *)kbox_usr, kbox_size)) {
		kfree(trs_key);
		kfree(wr_kbox);
		kfree(kbox);
		return SEC_HAL_RES_FAIL;
	}
	if (copy_from_user(trs_key, (const void __user *)trs_key_usr, trs_key_size)) {
		kfree(trs_key);
		kfree(wr_kbox);
		kfree(kbox);
		return SEC_HAL_RES_FAIL;
	...
	ret = sec_hal_drm_wrap_keybox((uint8_t *)kbox, kbox_size,
			(uint8_t *)wr_kbox, &wr_kbox_size, (uint8_t *)trs_key, trs_key_size);

	if (SEC_HAL_RES_OK == ret
			&& (wr_kbox_usr && copy_to_user((void __user *)wr_kbox_usr, wr_kbox, wr_kbox_size)))
		ret = SEC_HAL_RES_FAIL;
...
static inline
uint32_t _drm_install_keybox(sd_ioctl_params_t *params)
{
...
static inline
uint32_t _get_random(sd_ioctl_params_t *params)
{
	uint32_t ret, size;
	void *rand;
...
static inline
uint32_t _drm_get_device_id(sd_ioctl_params_t *params)
{
...
static inline
uint32_t _drm_get_keydata(sd_ioctl_params_t *params)
{
...
long sec_hal_drm_usr_init(void **drm_data)
{
	...
	do {
		if (*drm_data != NULL) { /* failsafe, double 'init' not allowed */
			rv = EFAULT;
			break;
		}
