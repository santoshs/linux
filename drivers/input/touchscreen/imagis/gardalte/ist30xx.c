...
#if defined(IST30XX_TRACKING_MODE) || defined(IST30XX_NOISE_MODE)
static struct timespec t_event;
#endif

#if IST30XX_NOISE_MODE
#define IST30XX_IDLE_STATUS     (0x1D4E0000)
#define IDLE_ALGORITHM_MODE     (1U << 15)
#define IDLE_TIMER_INTERVAL     (HZ)   // 1
...
#if 0
	printk("[ TSP ] [%08x] id: %d, event: %d, cdc: %d, anr: %d\n",
...
static irqreturn_t ist30xx_irq_thread(int irq, void *ptr)
{
	...
	{
		u32 ms = t_event.tv_sec * 1000 + t_event.tv_nsec / 1000000;
	...
	if (get_event_mode) {
		if ((msg[0] & 0xFFFF0000) == IST30XX_IDLE_STATUS) {
			...
			for (i = 0; i < IST30XX_MAX_MT_FINGERS; i++) {
				...
				if (data->fingers[i].bit_field.udmg & PRESS_MSG_MASK) {
					schedule_delayed_work(&work_reset_check, 0);
	...
	if (get_event_mode) {
		if (ts_data->status.power == 1) {
			...
			if (curr_ms - event_ms > 3000) {
				printk("[ TSP ] idle timeout over 3sec\n");
				schedule_delayed_work(&work_reset_check, 0);
			}
		}
	}
...
void fw_update(void *device_data)
{
	...
	ist30xx_parse_fw_ver(PARSE_FLAG_FW, fw);

	mutex_lock(&ist30xx_mutex);
	while (retry--) {
        ret = ist30xx_fw_param_update(ts_data->client, fw);
        if (ret)
            break;
	}
	...
	snprintf(buff, sizeof(buff), "%s","Complete");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	printk("[TSP] %s, fw_ver_from_binary=%d\n", __func__,tsp_fw_version);
...

{
	...
	printk("[TSP] %s, fw_ver_from_ic=%d\n", __func__, data->param_ver);
...
static DEVICE_ATTR(touchkey, S_IRWXUGO, fac_read_key_show, fac_read_key_store); // by IMAGIS
static DEVICE_ATTR(touchkey_menu, S_IRUGO, fac_read_diff_menu_show, NULL);
static DEVICE_ATTR(touchkey_back, S_IRUGO, fac_read_diff_back_show, NULL);
				   ...
				   const struct i2c_device_id * id)
{
	...
	if (ret) {
		printk("request_threaded_irq error!!!\n");
		goto err_irq;
	}
	ist30xx_disable_irq(data);
	while (retry--) {
	...
	ist30xx_initialize(data, ist30xx_param, sizeof(ist30xx_param));
	...
	ret = sysfs_create_group(&sec_touchkey->kobj, &fac_attr_group);
	if (ret)
