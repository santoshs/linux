...
int ist30xx_auto_fw_update(struct ist30xx_data *data)
{
	...
	if ((data->fw.ver > 0) && (data->fw.ver < 0xFFFFFFFF)) {
		...
		//if (data->fw.ver == ist30xx_fw_ver)
		{		
			printk("[ TSP ] fw.ver=0x%08x, ist30xx_fw_ver=0x%08x", data->fw.ver, ist30xx_fw_ver);
...
int ist30xx_auto_param_update(struct ist30xx_data *data)
{
	...
	if ((data->param_ver > 0) && (data->param_ver < 0xFFFFFFFF)) {
		...
		//if (data->param_ver == val)
		{
			printk("[ TSP ] param_ver=0x%04x, val=0x%04x\n",data->param_ver, val);
			return 0;
	        }
	}
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	if ((data->param_ver > 0) && (data->param_ver < 0xFFFFFFFF) &&
		...
		DMSG("[ TSP ] F/W: 0x%04x -> 0x%04x, Param: 0x%04x -> 0x%04x\n",
	...
	DMSG("[ TSP ] Update version F/W: 0x%08x -> 0x%08x, Param: 0x%04x -> 0x%04x\n",
	     data->fw.ver, ist30xx_fw_ver, data->param_ver, ist30xx_param_ver);
...
int ist30xx_initialize(struct ist30xx_data *data, const u8 *buf, int size)
{
	int ret;
	u32 len;
	u32 *cfg = (u32 *)buf;
	...
	/* init config */
	len = (ts_data->tags.cfg_size >> 2);
	ret = ist30xx_write_buf(client, CMD_UPDATE_CONFIG, cfg, len);
	update_next_step(ret);
	msleep(10);
	...
	/* init sensor */
	len = ((ts_data->tags.sensor1_size + ts_data->tags.sensor2_size) >> 2);
	ret = ist30xx_write_buf(client, CMD_UPDATE_SENSOR, cfg, len);
	update_next_step(ret);
	msleep(10);
	...
	if (size < 0)
		pr_err("[ TSP ] ist30xx init fail, size=%d\n", size);
	else if (ret)
		pr_err("[ TSP ] ist30xx init fail, ret=%d\n", ret);
			 ...
			 const char *buf, size_t size)
{
	...
	if (mode == MASK_UPDATE_ISP || mode == MASK_UPDATE_FW || mode == MASK_UPDATE_ALL) {
	...
	mutex_lock(&ist30xx_mutex);
	while (retry--) {
		if (mode == MASK_UPDATE_ALL) {
			ret = ist30xx_fw_param_update(ts_data->client, fw);
			if (!ret)
				break;
		} else if (mode == MASK_UPDATE_ISP || mode == MASK_UPDATE_FW || mode == 0) {
