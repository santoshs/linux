 ...
 * @dev: i2c_client device
 * @alarm: rtc_wkalrm struture, store the alarm configuration read from RTC register
 * return:
 *        0: Normal operation
 *       <0: Error occurs
 */
...
int tps80032_rtc_init_irq(struct device *dev, struct rtc_tps80032_data *data)
{
	...
	ret = request_threaded_irq(pdata->irq, NULL, tps80032_rtc_irq, IRQF_ONESHOT, "INT_RTC", dev);
	
	if(ret < 0) {
		dev_err(dev->parent, "request_irq failed \n");
		return ret;
	}
...
static int __devinit tps80032_rtc_probe(struct platform_device *pdev)
{
	...
	if (!(data = kzalloc(sizeof(struct rtc_tps80032_data), GFP_KERNEL)))
		return -ENOMEM;
	...
	/*If time is POR values, set initial time is 2012/02/01 */
