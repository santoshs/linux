...
int rreg_cmd(char*, int);
int wreg_cmd(char*, int);
int mreg_cmd(char*, int);
int rev_cmd(char*, int);
int hw_init(void);
void hw_exit(void);
...
int rreg_cmd(char *para, int size)
{
	...
	/*MSG("size: %d, address: %x, base: %x", reg_size, reg_addr, base_addr);*/
...
int wreg_cmd(char *para, int size)
{
	...
	vir_addr = NULL;
	ret = 0;
	sprintf(bufres, "0x%x: \nSet: 0x%x \nActual: 0x%x", reg_addr, set_value, val);
	/*MSG("Value: 0x%x", val);*/
	goto end;
...
{
	unsigned int ret = __raw_readl(CCCR);
	
	FUNC_MSG_IN;	
	
	switch (ret & CHIP_VERSION_MASK) {
