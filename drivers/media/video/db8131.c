...
static int DB8131_s_mbus_config(struct v4l2_subdev *sd,
...
static struct v4l2_subdev_video_ops DB8131_subdev_video_ops = {
	...
	.g_mbus_config	= DB8131_g_mbus_config,
	.s_mbus_config	= DB8131_s_mbus_config,
};
...
static int DB8131_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}

struct v4l2_ctrl_ops DB8131_ctrl_ops = {
	.s_ctrl = DB8131_s_ctrl,
};
			...
			const struct i2c_device_id *did)
{
	...
	if (!priv) {
		dev_err(&client->dev,
			"DB8131: Failed to allocate memory for private data!\n");
		return -ENOMEM;
	}

	v4l2_i2c_subdev_init(&priv->subdev, client, &DB8131_subdev_ops);
	v4l2_ctrl_handler_init(&priv->hdl, 4);
	v4l2_ctrl_new_std(&priv->hdl, &DB8131_ctrl_ops,
...
module_i2c_driver(DB8131_i2c_driver);
